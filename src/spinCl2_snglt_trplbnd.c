...
extern double U_spinCl2_snglt_trplbnd;					//declare structure energy                           

void specspinCl2_snglt_trplbnd(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cl2_snglt_trplbnd");			//Enter full name for the structure
	strcpy(spec->name, "Cl2_sn_tr");				//Enter abbreviation name
	strcpy(spec->spin, "a7b7");		//Enter spin structure
	spec->type = Cl2_snglt_trplbnd;				//Enter type name
	...
	spec->ainter = ainterspinCl2_snglt_trplbnd;			//Enter the function name for "ainter" 
	spec->dev = devspinCl2_snglt_trplbnd;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//calculate structure energy
        if((MC_spinCl2_snglt_trplbnd) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCl2_snglt_trplbnd);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCl2_snglt_trplbnd)){ output(spec->coord, spec->numPart, Out); }
