...
extern double U_spinClplus1ex;					//declare structure energy                           

void specspinClplus1ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Clplus1ex");			//Enter full name for the structure
	strcpy(spec->name, "Cl+1*");				//Enter abbreviation name
	strcpy(spec->spin, "a3b3");		//Enter spin structure
	spec->type = Clplus1ex;				//Enter type name
	...
	spec->ainter = ainterspinClplus1ex;			//Enter the function name for "ainter" 
	spec->dev = devspinClplus1ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinClplus1ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinClplus1ex)){ output(spec->coord, spec->numPart, Out); }
