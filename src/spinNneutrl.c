...
extern double U_spinNneutrl;					//declare structure energy                           

void specspinNneutrl(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Nneutrl");			//Enter full name for the structure
	strcpy(spec->name, "N+0");				//Enter abbreviation name
	strcpy(spec->spin, "a4b1");		//Enter spin structure
	spec->type = Nneutrl;				//Enter type name
	...
	spec->ainter = ainterspinNneutrl;			//Enter the function name for "ainter" 
	spec->dev = devspinNneutrl;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNneutrl);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNneutrl)){ output(spec->coord, spec->numPart, Out); }
