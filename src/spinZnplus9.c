...
extern double U_spinZnplus9;					//declare structure energy                           

void specspinZnplus9(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus9");			//Enter full name for the structure
	strcpy(spec->name, "Zn+9");				//Enter abbreviation name
	strcpy(spec->spin, "a3b0");		//Enter spin structure
	spec->type = Znplus9;				//Enter type name
	...
	spec->ainter = ainterspinZnplus9;			//Enter the function name for "ainter" 
	spec->dev = devspinZnplus9;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnplus9);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus9)){ output(spec->coord, spec->numPart, Out); }
