...
extern double U_spinZnplus2ex2;					//declare structure energy                           

void specspinZnplus2ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus2ex2");			//Enter full name for the structure
	strcpy(spec->name, "Zn+2^^");				//Enter abbreviation name
	strcpy(spec->spin, "a7b3");		//Enter spin structure
	spec->type = Znplus2ex2;				//Enter type name
	...
	spec->ainter = ainterspinZnplus2ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinZnplus2ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//a-4 
       ...
       //b-1 
        Vin[7+1].x = rot(rb1, 0.0, 0.0, a, b, c, 0);
        Vin[7+1].y = rot(rb1, 0.0, 0.0, a, b, c, 1);
        Vin[7+1].z = rot(rb1, 0.0, 0.0, a, b, c, 2);
        Vin[7+1].q = -1;
        Vin[7+1].s = -2;
       ...
       //b-2 
        Vin[7+2].x = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[7+2].y = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[7+2].z = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[7+2].q = -1;
        Vin[7+2].s = -2;
       ...
       //b-3 
        Vin[7+3].x = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[7+3].y = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[7+3].z = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[7+3].q = -1;
        Vin[7+3].s = -2;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnplus2ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus2ex2)){ output(spec->coord, spec->numPart, Out); }
