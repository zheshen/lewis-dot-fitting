...
extern double U_spinNplus1ex2;					//declare structure energy                           

void specspinNplus1ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Nplus1ex2");			//Enter full name for the structure
	strcpy(spec->name, "N+1**");				//Enter abbreviation name
	strcpy(spec->spin, "a4b0");		//Enter spin structure
	spec->type = Nplus1ex2;				//Enter type name
	...
	spec->ainter = ainterspinNplus1ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinNplus1ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNplus1ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNplus1ex2)){ output(spec->coord, spec->numPart, Out); }
