...
extern double U_spinAlplus1;					//declare structure energy                           

void specspinAlplus1(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Alplus1");			//Enter full name for the structure
	strcpy(spec->name, "Al+1");				//Enter abbreviation name
	strcpy(spec->spin, "a1b1");		//Enter spin structure
	spec->type = Alplus1;				//Enter type name
	...
	spec->ainter = ainterspinAlplus1;			//Enter the function name for "ainter" 
	spec->dev = devspinAlplus1;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinAlplus1);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinAlplus1)){ output(spec->coord, spec->numPart, Out); }
