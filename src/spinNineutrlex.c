...
extern double U_spinNineutrlex;					//declare structure energy                           

void specspinNineutrlex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Nineutrlex");			//Enter full name for the structure
	strcpy(spec->name, "Ni+0*");				//Enter abbreviation name
	strcpy(spec->spin, "a5b5");		//Enter spin structure
	spec->type = Nineutrlex;				//Enter type name
	...
	spec->numPart = 11;					//Enter number of particles for the structure
	spec->stepSize = 0.101;					//Enter the stepsize for optimization, default is 0.101
	spec->ainter = ainterspinNineutrlex;			//Enter the function name for "ainter" 
	spec->dev = devspinNineutrlex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
       ...
       //b-1 
        Vin[5+1].x = rot(0.0, rb1, 0.0, a, b, c, 0);
        Vin[5+1].y = rot(0.0, rb1, 0.0, a, b, c, 1);
        Vin[5+1].z = rot(0.0, rb1, 0.0, a, b, c, 2);
        Vin[5+1].q = -1;
        Vin[5+1].s = -2;
        Vin[5+1].d = db1_r+db1_i*I;
        Vin[5+1].eta = db2;
        Vin[5+1].zeta = db3;

       //b-2 
        Vin[5+2].x = rot(0.0, -rb1, 0.0, a, b, c, 0);
        Vin[5+2].y = rot(0.0, -rb1, 0.0, a, b, c, 1);
        Vin[5+2].z = rot(0.0, -rb1, 0.0, a, b, c, 2);
        Vin[5+2].q = -1;
        Vin[5+2].s = -2;
        Vin[5+2].d = db1_r+db1_i*I;
        Vin[5+2].eta = db2;
        Vin[5+2].zeta = db3;

       //b-3 
        Vin[5+3].x = rot(rb1, 0.0, 0.0, a, b, c, 0);
        Vin[5+3].y = rot(rb1, 0.0, 0.0, a, b, c, 1);
        Vin[5+3].z = rot(rb1, 0.0, 0.0, a, b, c, 2);
        Vin[5+3].q = -1;
        Vin[5+3].s = -2;
        Vin[5+3].d = db1_r+db1_i*I;
        Vin[5+3].eta = db2;
        Vin[5+3].zeta = db3;

       //b-4 
        Vin[5+4].x = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[5+4].y = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[5+4].z = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[5+4].q = -1;
        Vin[5+4].s = -2;
        Vin[5+4].d = db1_r+db1_i*I;
        Vin[5+4].eta = db2;
        Vin[5+4].zeta = db3;

       //b-5 
        Vin[5+5].x = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[5+5].y = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[5+5].z = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[5+5].q = -1;
        Vin[5+5].s = -2;
        Vin[5+5].d = db1_r+db1_i*I;
        Vin[5+5].eta = db2;
        Vin[5+5].zeta = db3;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNineutrlex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNineutrlex)){ output(spec->coord, spec->numPart, Out); }
