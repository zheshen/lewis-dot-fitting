...
extern double U_spinCuplus3ex2;					//declare structure energy                           

void specspinCuplus3ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus3ex2");			//Enter full name for the structure
	strcpy(spec->name, "Cu+3**");				//Enter abbreviation name
	strcpy(spec->spin, "a6b2");		//Enter spin structure
	spec->type = Cuplus3ex2;				//Enter type name
	...
	spec->ainter = ainterspinCuplus3ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus3ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus3ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus3ex2)){ output(spec->coord, spec->numPart, Out); }
