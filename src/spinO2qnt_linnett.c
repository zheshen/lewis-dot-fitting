...
extern double U_spinO2qnt_linnett;					//declare structure energy                           

void specspinO2qnt_linnett(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "O2qnt_linnett");			//Enter full name for the structure
	strcpy(spec->name, "O2qnt_li");				//Enter abbreviation name
	strcpy(spec->spin, "a8b4");		//Enter spin structure
	spec->type = O2qnt_linnett;				//Enter type name
	...
	spec->ainter = ainterspinO2qnt_linnett;			//Enter the function name for "ainter" 
	spec->dev = devspinO2qnt_linnett;			//Enter the function name for "dev"
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinO2qnt_linnett);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinO2qnt_linnett)){ output(spec->coord, spec->numPart, Out); }
