...
extern double U_spinNiplus1ex;					//declare structure energy                           

void specspinNiplus1ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus1ex");			//Enter full name for the structure
	strcpy(spec->name, "Ni+1*");				//Enter abbreviation name
	strcpy(spec->spin, "a6b3");		//Enter spin structure
	spec->type = Niplus1ex;				//Enter type name
	...
	spec->ainter = ainterspinNiplus1ex;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus1ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus1ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus1ex)){ output(spec->coord, spec->numPart, Out); }
