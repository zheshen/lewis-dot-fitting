...
extern double U_spinZnneutrlex2;					//declare structure energy                           

void specspinZnneutrlex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znneutrlex2");			//Enter full name for the structure
	strcpy(spec->name, "Zn+0^^");				//Enter abbreviation name
	strcpy(spec->spin, "a8b4");		//Enter spin structure
	spec->type = Znneutrlex2;				//Enter type name
	...
	spec->ainter = ainterspinZnneutrlex2;			//Enter the function name for "ainter" 
	spec->dev = devspinZnneutrlex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//a-4 
       ...
       //a-8 
        ...
        Vin[8].zeta = da3;
       //b-1 
        Vin[8+1].x = rot(rb1, rb1, rb1, a, b, c, 0);
        Vin[8+1].y = rot(rb1, rb1, rb1, a, b, c, 1);
        Vin[8+1].z = rot(rb1, rb1, rb1, a, b, c, 2);
        Vin[8+1].q = -1;
        Vin[8+1].s = -2;
       ...
       //b-2 
        Vin[8+2].x = rot(rb1, -rb1, -rb1, a, b, c, 0);
        Vin[8+2].y = rot(rb1, -rb1, -rb1, a, b, c, 1);
        Vin[8+2].z = rot(rb1, -rb1, -rb1, a, b, c, 2);
        Vin[8+2].q = -1;
        Vin[8+2].s = -2;
       ...
       //b-3 
        Vin[8+3].x = rot(-rb1, rb1, -rb1, a, b, c, 0);
        Vin[8+3].y = rot(-rb1, rb1, -rb1, a, b, c, 1);
        Vin[8+3].z = rot(-rb1, rb1, -rb1, a, b, c, 2);
        Vin[8+3].q = -1;
        Vin[8+3].s = -2;
       ...
       //b-4 
        Vin[8+4].x = rot(-rb1, -rb1, rb1, a, b, c, 0);
        Vin[8+4].y = rot(-rb1, -rb1, rb1, a, b, c, 1);
        Vin[8+4].z = rot(-rb1, -rb1, rb1, a, b, c, 2);
        Vin[8+4].q = -1;
        Vin[8+4].s = -2;
        Vin[8+4].d = db1_r+db1_i*I;
        Vin[8+4].eta = db2;
        Vin[8+4].zeta = db3;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnneutrlex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnneutrlex2)){ output(spec->coord, spec->numPart, Out); }
