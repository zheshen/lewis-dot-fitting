...
extern double U_spinP2_snglt_linnett;					//declare structure energy                           

void specspinP2_snglt_linnett(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "P2_snglt_linnett");			//Enter full name for the structure
	strcpy(spec->name, "P2_sn_li");				//Enter abbreviation name
	strcpy(spec->spin, "a5b5");		//Enter spin structure
	spec->type = P2_snglt_linnett;				//Enter type name
	...
	spec->ainter = ainterspinP2_snglt_linnett;			//Enter the function name for "ainter" 
	spec->dev = devspinP2_snglt_linnett;			//Enter the function name for "dev"
...
Vin[11].s = -2;
	...
	//calculate structure energy
        if((MC_spinP2_snglt_linnett) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinP2_snglt_linnett);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinP2_snglt_linnett)){ output(spec->coord, spec->numPart, Out); }
