...
extern double U_spinCuplus8;					//declare structure energy                           

void specspinCuplus8(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus8");			//Enter full name for the structure
	strcpy(spec->name, "Cu+8");				//Enter abbreviation name
	strcpy(spec->spin, "a3b0");		//Enter spin structure
	spec->type = Cuplus8;				//Enter type name
	...
	spec->ainter = ainterspinCuplus8;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus8;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus8);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus8)){ output(spec->coord, spec->numPart, Out); }
