...
extern double U_spinCuplus5;					//declare structure energy                           

void specspinCuplus5(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus5");			//Enter full name for the structure
	strcpy(spec->name, "Cu+5");				//Enter abbreviation name
	strcpy(spec->spin, "a5b1");		//Enter spin structure
	spec->type = Cuplus5;				//Enter type name
	...
	spec->ainter = ainterspinCuplus5;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus5;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus5);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus5)){ output(spec->coord, spec->numPart, Out); }
