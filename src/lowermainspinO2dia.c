...
double lowermainspinO2dia(){
	...
	char molecule[] = "OXYGEN  DIATOMIC SNG MIN";
	...
	Species spec;
	spec.icue = 4;
	strcpy(spec.fullname, "O2dia");			//Enter full name for the structure
	strcpy(spec.name, "O2dia");				//Enter abbreviation name
	strcpy(spec.spin, "a6b6");		//Enter spin structure
	spec.type = O2dia;				//Enter type name
	...
	Species dblbnd;
	Species trplbnd;
	linnett.type = O2dia_linnett;
	snglbnd.type = O2dia_snglbnd;
	dblbnd.type = O2dia_dblbnd;
	trplbnd.type = O2dia_trplbnd;
	specSwitch(&linnett);
	specSwitch(&snglbnd);
	specSwitch(&dblbnd);
	specSwitch(&trplbnd);
...
#pragma omp section 
					...
					(*linnett.ainter)(&linnett);
					else U_spinO2dia_linnett = 0.0;
				}
#pragma omp section 
				{
					if (wspinO2dia_snglbnd)
					(*snglbnd.ainter)(&snglbnd);
					else U_spinO2dia_snglbnd = 0.0;
				}
#pragma omp section 
				{
					if (wspinO2dia_dblbnd)
					(*dblbnd.ainter)(&dblbnd);
					else U_spinO2dia_dblbnd = 0.0;
				}
#pragma omp section 
				{
					if (wspinO2dia_trplbnd)
      					(*trplbnd.ainter)(&trplbnd);
					else U_spinO2dia_trplbnd = 0.0;
				}
			}
			}


//					U_spinO2dia_linnett = (*linnett.ainter)(&linnett);
//					U_spinO2dia_snglbnd = (*snglbnd.ainter)(&snglbnd);
//					U_spinO2dia_dblbnd = (*dblbnd.ainter)(&dblbnd);
//					U_spinO2dia_trplbnd = (*trplbnd.ainter)(&trplbnd);
//					printf("%f, %f, %f, %f\n", U_spinO2dia_linnett, U_spinO2dia_snglbnd, U_spinO2dia_dblbnd, U_spinO2dia_trplbnd);

			U_spinO2dia=fmin(fmin(fmin(U_spinO2dia_linnett,U_spinO2dia_snglbnd),U_spinO2dia_dblbnd),U_spinO2dia_trplbnd); //spinO2dia (triplet)
			if (U_spinO2dia==U_spinO2dia_linnett)
			{
			...
			else if(U_spinO2dia==U_spinO2dia_snglbnd)
			{
			...
			else if(U_spinO2dia==U_spinO2dia_dblbnd)
			{
			...
			else if(U_spinO2dia==U_spinO2dia_trplbnd)
			{
...
//				deviation = 5E5;
        ...
        fprintf(OutFile, "| Potential Energy      |   kJ/mol   |      -         |   -   |   Linnett      |  % 7.5e  |", U_spinO2dia_linnett );
        fprintf(OutFile, "   N/A   |       -       |\n");

        fprintf(OutFile, "| Potential Energy      |   kJ/mol   |      -         |   -   |   Snglbnd      |  % 7.5e  |", U_spinO2dia_snglbnd );
        fprintf(OutFile, "   N/A   |       -       |\n");

        fprintf(OutFile, "| Potential Energy      |   kJ/mol   |      -         |   -   |   Dblbnd       |  % 7.5e  |", U_spinO2dia_dblbnd );
        fprintf(OutFile, "   N/A   |       -       |\n");

        fprintf(OutFile, "| Potential Energy      |   kJ/mol   |      -         |   -   |   Trplbnd      |  % 7.5e  |", U_spinO2dia_trplbnd );
        fprintf(OutFile, "   N/A   |       -       |\n");
				...
				printEnergy(U_spinO2dia);

                        extern int writefinalpdb;
                        if ((writefinalpdb)&&(spec.icue==4)){output(spec.coord, spec.numPart, "spinO2dia_min_final.pdb");}
