...
extern double U_spinMnneutrlex;					//declare structure energy                           

void specspinMnneutrlex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Mnneutrlex");			//Enter full name for the structure
	strcpy(spec->name, "Mn+0*");				//Enter abbreviation name
	strcpy(spec->spin, "a7b0");		//Enter spin structure
	spec->type = Mnneutrlex;				//Enter type name
	...
	spec->ainter = ainterspinMnneutrlex;			//Enter the function name for "ainter" 
	spec->dev = devspinMnneutrlex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinMnneutrlex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinMnneutrlex)){ output(spec->coord, spec->numPart, Out); }
