...
extern double U_spinSplus2;					//declare structure energy                           

void specspinSplus2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Splus2");			//Enter full name for the structure
	strcpy(spec->name, "S+2");				//Enter abbreviation name
	strcpy(spec->spin, "a3b1");		//Enter spin structure
	spec->type = Splus2;				//Enter type name
	...
	spec->ainter = ainterspinSplus2;			//Enter the function name for "ainter" 
	spec->dev = devspinSplus2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinSplus2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinSplus2)){ output(spec->coord, spec->numPart, Out); }
