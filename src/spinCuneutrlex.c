...
extern double U_spinCuneutrlex;					//declare structure energy                           

void specspinCuneutrlex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuneutrlex");			//Enter full name for the structure
	strcpy(spec->name, "Cu+0*");				//Enter abbreviation name
	strcpy(spec->spin, "a7b4");		//Enter spin structure
	spec->type = Cuneutrlex;				//Enter type name
	...
	spec->ainter = ainterspinCuneutrlex;			//Enter the function name for "ainter" 
	spec->dev = devspinCuneutrlex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//a-4 
       ...
       //b-1 
        Vin[7+1].x = rot(rb1, rb1, rb1, a, b, c, 0);
        Vin[7+1].y = rot(rb1, rb1, rb1, a, b, c, 1);
        Vin[7+1].z = rot(rb1, rb1, rb1, a, b, c, 2);
        Vin[7+1].q = -1;
        Vin[7+1].s = -2;
       ...
       //b-2 
        Vin[7+2].x = rot(rb1, -rb1, -rb1, a, b, c, 0);
        Vin[7+2].y = rot(rb1, -rb1, -rb1, a, b, c, 1);
        Vin[7+2].z = rot(rb1, -rb1, -rb1, a, b, c, 2);
        Vin[7+2].q = -1;
        Vin[7+2].s = -2;
       ...
       //b-3 
        Vin[7+3].x = rot(-rb1, rb1, -rb1, a, b, c, 0);
        Vin[7+3].y = rot(-rb1, rb1, -rb1, a, b, c, 1);
        Vin[7+3].z = rot(-rb1, rb1, -rb1, a, b, c, 2);
        Vin[7+3].q = -1;
        Vin[7+3].s = -2;
       ...
       //b-4 
        Vin[7+4].x = rot(-rb1, -rb1, rb1, a, b, c, 0);
        Vin[7+4].y = rot(-rb1, -rb1, rb1, a, b, c, 1);
        Vin[7+4].z = rot(-rb1, -rb1, rb1, a, b, c, 2);
        Vin[7+4].q = -1;
        Vin[7+4].s = -2;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuneutrlex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuneutrlex)){ output(spec->coord, spec->numPart, Out); }
