...
extern double U_spinNiplus4ex2;					//declare structure energy                           

void specspinNiplus4ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus4ex2");			//Enter full name for the structure
	strcpy(spec->name, "Ni+4**");				//Enter abbreviation name
	strcpy(spec->spin, "a3b3");		//Enter spin structure
	spec->type = Niplus4ex2;				//Enter type name
	...
	spec->ainter = ainterspinNiplus4ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus4ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus4ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus4ex2)){ output(spec->coord, spec->numPart, Out); }
