...
extern double U_spinNiplus5ex;					//declare structure energy                           

void specspinNiplus5ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus5ex");			//Enter full name for the structure
	strcpy(spec->name, "Ni+5*");				//Enter abbreviation name
	strcpy(spec->spin, "a4b1");		//Enter spin structure
	spec->type = Niplus5ex;				//Enter type name
	...
	spec->ainter = ainterspinNiplus5ex;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus5ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus5ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus5ex)){ output(spec->coord, spec->numPart, Out); }
