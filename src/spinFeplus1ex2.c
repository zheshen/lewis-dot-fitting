...
extern double U_spinFeplus1ex2;					//declare structure energy                           

void specspinFeplus1ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Feplus1ex2");			//Enter full name for the structure
	strcpy(spec->name, "Fe+1**");				//Enter abbreviation name
	strcpy(spec->spin, "a6b1");		//Enter spin structure
	spec->type = Feplus1ex2;				//Enter type name
	...
	spec->ainter = ainterspinFeplus1ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinFeplus1ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFeplus1ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFeplus1ex2)){ output(spec->coord, spec->numPart, Out); }
