...
extern double U_spinOplus5;					//declare structure energy                           

void specspinOplus5(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Oplus5");			//Enter full name for the structure
	strcpy(spec->name, "O+5");				//Enter abbreviation name
	strcpy(spec->spin, "a1b0");		//Enter spin structure
	spec->type = Oplus5;				//Enter type name
	...
	spec->ainter = ainterspinOplus5;			//Enter the function name for "ainter" 
	spec->dev = devspinOplus5;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinOplus5);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinOplus5)){ output(spec->coord, spec->numPart, Out); }
