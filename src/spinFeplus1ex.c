...
extern double U_spinFeplus1ex;					//declare structure energy                           

void specspinFeplus1ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Feplus1ex");			//Enter full name for the structure
	strcpy(spec->name, "Fe+1*");				//Enter abbreviation name
	strcpy(spec->spin, "a5b2");		//Enter spin structure
	spec->type = Feplus1ex;				//Enter type name
	...
	spec->ainter = ainterspinFeplus1ex;			//Enter the function name for "ainter" 
	spec->dev = devspinFeplus1ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFeplus1ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFeplus1ex)){ output(spec->coord, spec->numPart, Out); }
