...
extern double U_spinBneutrlex;					//declare structure energy                           

void specspinBneutrlex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Bneutrlex");			//Enter full name for the structure
	strcpy(spec->name, "B+0*");				//Enter abbreviation name
	strcpy(spec->spin, "a3b0");		//Enter spin structure
	spec->type = Bneutrlex;				//Enter type name
	...
	spec->ainter = ainterspinBneutrlex;			//Enter the function name for "ainter" 
	spec->dev = devspinBneutrlex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinBneutrlex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinBneutrlex)){ output(spec->coord, spec->numPart, Out); }
