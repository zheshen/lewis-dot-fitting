...
extern double U_spinNiplus5ex2;					//declare structure energy                           

void specspinNiplus5ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus5ex2");			//Enter full name for the structure
	strcpy(spec->name, "Ni+5**");				//Enter abbreviation name
	strcpy(spec->spin, "a3b2");		//Enter spin structure
	spec->type = Niplus5ex2;				//Enter type name
	...
	spec->ainter = ainterspinNiplus5ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus5ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus5ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus5ex2)){ output(spec->coord, spec->numPart, Out); }
