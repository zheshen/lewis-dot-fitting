...
extern double U_spinP2_snglt_snglbnd;					//declare structure energy                           

void specspinP2_snglt_snglbnd(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "P2_snglt_snglbnd");			//Enter full name for the structure
	strcpy(spec->name, "P2_sn_sn");				//Enter abbreviation name
	strcpy(spec->spin, "a5b5");		//Enter spin structure
	spec->type = P2_snglt_snglbnd;				//Enter type name
	...
	spec->ainter = ainterspinP2_snglt_snglbnd;			//Enter the function name for "ainter" 
	spec->dev = devspinP2_snglt_snglbnd;			//Enter the function name for "dev"
...
Vin[11].s = -2;
	...
	//calculate structure energy
        if((MC_spinP2_snglt_snglbnd) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinP2_snglt_snglbnd);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinP2_snglt_snglbnd)){ output(spec->coord, spec->numPart, Out); }
