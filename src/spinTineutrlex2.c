...
extern double U_spinTineutrlex2;					//declare structure energy                           

void specspinTineutrlex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Tineutrlex2");			//Enter full name for the structure
	strcpy(spec->name, "Ti+0**");				//Enter abbreviation name
	strcpy(spec->spin, "a2b2");		//Enter spin structure
	spec->type = Tineutrlex2;				//Enter type name
	...
	spec->numPart = 5;					//Enter number of particles for the structure
	spec->stepSize = 0.101;					//Enter the stepsize for optimization, default is 0.101
	spec->ainter = ainterspinTineutrlex2;			//Enter the function name for "ainter" 
	spec->dev = devspinTineutrlex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
       ...
       //b-1 
        Vin[2+1].x = rot(0.0, rb1, 0.0, a, b, c, 0);
        Vin[2+1].y = rot(0.0, rb1, 0.0, a, b, c, 1);
        Vin[2+1].z = rot(0.0, rb1, 0.0, a, b, c, 2);
        Vin[2+1].q = -1;
        Vin[2+1].s = -2;
       ...
       //b-2 
        Vin[2+2].x = rot(0.0, -rb1, 0.0, a, b, c, 0);
        Vin[2+2].y = rot(0.0, -rb1, 0.0, a, b, c, 1);
        Vin[2+2].z = rot(0.0, -rb1, 0.0, a, b, c, 2);
        Vin[2+2].q = -1;
        Vin[2+2].s = -2;
        Vin[2+2].d = db1_r+db1_i*I;
        Vin[2+2].eta = db2;
        Vin[2+2].zeta = db3;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinTineutrlex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinTineutrlex2)){ output(spec->coord, spec->numPart, Out); }
