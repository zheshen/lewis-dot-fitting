...
extern double U_spinScplus2;					//declare structure energy                           

void specspinScplus2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Scplus2");			//Enter full name for the structure
	strcpy(spec->name, "Sc+2");				//Enter abbreviation name
	strcpy(spec->spin, "a1b0");		//Enter spin structure
	spec->type = Scplus2;				//Enter type name
	...
	spec->ainter = ainterspinScplus2;			//Enter the function name for "ainter" 
	spec->dev = devspinScplus2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinScplus2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinScplus2)){ output(spec->coord, spec->numPart, Out); }
