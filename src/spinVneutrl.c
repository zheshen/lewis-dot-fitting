...
extern double U_spinVneutrl;					//declare structure energy                           

void specspinVneutrl(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Vneutrl");			//Enter full name for the structure
	strcpy(spec->name, "V+0");				//Enter abbreviation name
	strcpy(spec->spin, "a4b1");		//Enter spin structure
	spec->type = Vneutrl;				//Enter type name
	...
	spec->ainter = ainterspinVneutrl;			//Enter the function name for "ainter" 
	spec->dev = devspinVneutrl;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinVneutrl);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinVneutrl)){ output(spec->coord, spec->numPart, Out); }
