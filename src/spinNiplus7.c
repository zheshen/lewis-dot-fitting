...
extern double U_spinNiplus7;					//declare structure energy                           

void specspinNiplus7(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus7");			//Enter full name for the structure
	strcpy(spec->name, "Ni+7");				//Enter abbreviation name
	strcpy(spec->spin, "a3b0");		//Enter spin structure
	spec->type = Niplus7;				//Enter type name
	...
	spec->ainter = ainterspinNiplus7;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus7;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus7);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus7)){ output(spec->coord, spec->numPart, Out); }
