...
extern double U_spinDielectron;					//declare structure energy                           

void specspinDielectron(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Dielectron");			//Enter full name for the structure
	strcpy(spec->name, "e2");				//Enter abbreviation name
	strcpy(spec->spin, "a2b0");		//Enter spin structure
	spec->type = Dielectron;				//Enter type name
	...
	spec->ainter = ainterspinDielectron;			//Enter the function name for "ainter" 
	spec->dev = devspinDielectron;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//calculate structure energy
        if((MC_spinDielectron) && (spec->icue ==4)){
	...
	U_spinDielectron = Usum;
//	printf("U_spinDielectron=%f\n",U_spinDielectron);
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<ABOVE CHANGABLE<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
//                         fprintf(OutFile,"| Lone B1 radius        |      A     |      -         |   -   |                |    % 8.5f    |   N/A   |       -       |\n", fd2);
	    	...
	    	fclose(OutFile);
	printf("U_spinDielectron=%f\n",U_spinDielectron);
		printEnergy(U_spinDielectron);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinDielectron)){ output(spec->coord, spec->numPart, Out); }
