...
extern double U_spinPneutrlex;					//declare structure energy                           

void specspinPneutrlex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Pneutrlex");			//Enter full name for the structure
	strcpy(spec->name, "P+0*");				//Enter abbreviation name
	strcpy(spec->spin, "a3b2");		//Enter spin structure
	spec->type = Pneutrlex;				//Enter type name
	...
	spec->ainter = ainterspinPneutrlex;			//Enter the function name for "ainter" 
	spec->dev = devspinPneutrlex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinPneutrlex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinPneutrlex)){ output(spec->coord, spec->numPart, Out); }
