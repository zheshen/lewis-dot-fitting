...
extern double U_spinS2_trplt_linnett;					//declare structure energy                           

void specspinS2_trplt_linnett(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "S2_trplt_linnett");			//Enter full name for the structure
	strcpy(spec->name, "S2_tr_li");				//Enter abbreviation name
	strcpy(spec->spin, "a7b5");		//Enter spin structure
	spec->type = S2_trplt_linnett;				//Enter type name
	...
	spec->ainter = ainterspinS2_trplt_linnett;			//Enter the function name for "ainter" 
	spec->dev = devspinS2_trplt_linnett;			//Enter the function name for "dev"
...
Vin[13].s = -2;
	...
	//calculate structure energy
        if((MC_spinS2_trplt_linnett) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinS2_trplt_linnett);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinS2_trplt_linnett)){ output(spec->coord, spec->numPart, Out); }
