...
extern double U_spinH3plus;					//declare structure energy                           

void specspinH3plus(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "H3plus");			//Enter full name for the structure
	strcpy(spec->name, "H3+1");				//Enter abbreviation name
	strcpy(spec->spin, "a1b1");		//Enter spin structure
	spec->type = H3plus;				//Enter type name
	...
	spec->ainter = ainterspinH3plus;			//Enter the function name for "ainter" 
	spec->dev = devspinH3plus;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                if (MC_spinH3plus){
			fprintf(OutFile,"| Kernel  dist1         |      A     |    % 8.5f    |   -   |        -       |    %8.5f    |   N/A   |       -       |\n", real_bl, fr1);
			fprintf(OutFile,"| Kernel  dist2         |      A     |    % 8.5f    |   -   |        -       |    %8.5f    |   N/A   |       -       |\n", real_bl, fr2);
			fprintf(OutFile,"| Kernel  dist3         |      A     |    % 8.5f    |   -   |        -       |    %8.5f    |   N/A   |       -       |\n", real_bl, fr3);
			...
			//fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinH3plus);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinH3plus)){ output(spec->coord, spec->numPart, Out); }
