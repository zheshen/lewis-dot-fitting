...
extern double U_spinNiplus2;					//declare structure energy                           

void specspinNiplus2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus2");			//Enter full name for the structure
	strcpy(spec->name, "Ni+2");				//Enter abbreviation name
	strcpy(spec->spin, "a5b3");		//Enter spin structure
	spec->type = Niplus2;				//Enter type name
	...
	spec->ainter = ainterspinNiplus2;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus2)){ output(spec->coord, spec->numPart, Out); }
