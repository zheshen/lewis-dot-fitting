...
extern double U_spinFneutrl;					//declare structure energy                           

void specspinFneutrl(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Fneutrl");			//Enter full name for the structure
	strcpy(spec->name, "F+0");				//Enter abbreviation name
	strcpy(spec->spin, "a4b3");		//Enter spin structure
	spec->type = Fneutrl;				//Enter type name
	...
	spec->ainter = ainterspinFneutrl;			//Enter the function name for "ainter" 
	spec->dev = devspinFneutrl;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFneutrl);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFneutrl)){ output(spec->coord, spec->numPart, Out); }
