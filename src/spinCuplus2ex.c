...
extern double U_spinCuplus2ex;					//declare structure energy                           

void specspinCuplus2ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus2ex");			//Enter full name for the structure
	strcpy(spec->name, "Cu+2*");				//Enter abbreviation name
	strcpy(spec->spin, "a6b3");		//Enter spin structure
	spec->type = Cuplus2ex;				//Enter type name
	...
	spec->ainter = ainterspinCuplus2ex;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus2ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus2ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus2ex)){ output(spec->coord, spec->numPart, Out); }
