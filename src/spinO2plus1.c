...
extern double U_spinO2plus1;					//declare structure energy                           

void specspinO2plus1(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "O2plus1");			//Enter full name for the structure
	strcpy(spec->name, "O2+1");				//Enter abbreviation name
	strcpy(spec->spin, "a6b5");		//Enter spin structure
	spec->type = O2plus1;				//Enter type name
	...
	spec->ainter = ainterspinO2plus1;			//Enter the function name for "ainter" 
	spec->dev = devspinO2plus1;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	double fr1, bo;
	const double real_bl = 1.123;
	const double real_bo = 2.5; 
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinO2plus1);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinO2plus1)){ output(spec->coord, spec->numPart, Out); }
