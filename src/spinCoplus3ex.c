...
extern double U_spinCoplus3ex;					//declare structure energy                           

void specspinCoplus3ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Coplus3ex");			//Enter full name for the structure
	strcpy(spec->name, "Co+3*");				//Enter abbreviation name
	strcpy(spec->spin, "a4b2");		//Enter spin structure
	spec->type = Coplus3ex;				//Enter type name
	...
	spec->ainter = ainterspinCoplus3ex;			//Enter the function name for "ainter" 
	spec->dev = devspinCoplus3ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCoplus3ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCoplus3ex)){ output(spec->coord, spec->numPart, Out); }
