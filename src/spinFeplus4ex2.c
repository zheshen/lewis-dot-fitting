...
extern double U_spinFeplus4ex2;					//declare structure energy                           

void specspinFeplus4ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Feplus4ex2");			//Enter full name for the structure
	strcpy(spec->name, "Fe+4**");				//Enter abbreviation name
	strcpy(spec->spin, "a2b2");		//Enter spin structure
	spec->type = Feplus4ex2;				//Enter type name
	...
	spec->ainter = ainterspinFeplus4ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinFeplus4ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFeplus4ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFeplus4ex2)){ output(spec->coord, spec->numPart, Out); }
