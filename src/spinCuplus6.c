...
extern double U_spinCuplus6;					//declare structure energy                           

void specspinCuplus6(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus6");			//Enter full name for the structure
	strcpy(spec->name, "Cu+6");				//Enter abbreviation name
	strcpy(spec->spin, "a5b0");		//Enter spin structure
	spec->type = Cuplus6;				//Enter type name
	...
	spec->ainter = ainterspinCuplus6;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus6;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus6);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus6)){ output(spec->coord, spec->numPart, Out); }
