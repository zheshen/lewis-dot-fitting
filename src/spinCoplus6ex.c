...
extern double U_spinCoplus6ex;					//declare structure energy                           

void specspinCoplus6ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Coplus6ex");			//Enter full name for the structure
	strcpy(spec->name, "Co+6*");				//Enter abbreviation name
	strcpy(spec->spin, "a2b1");		//Enter spin structure
	spec->type = Coplus6ex;				//Enter type name
	...
	spec->ainter = ainterspinCoplus6ex;			//Enter the function name for "ainter" 
	spec->dev = devspinCoplus6ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCoplus6ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCoplus6ex)){ output(spec->coord, spec->numPart, Out); }
