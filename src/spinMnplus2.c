...
extern double U_spinMnplus2;					//declare structure energy                           

void specspinMnplus2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Mnplus2");			//Enter full name for the structure
	strcpy(spec->name, "Mn+2");				//Enter abbreviation name
	strcpy(spec->spin, "a5b0");		//Enter spin structure
	spec->type = Mnplus2;				//Enter type name
	...
	spec->ainter = ainterspinMnplus2;			//Enter the function name for "ainter" 
	spec->dev = devspinMnplus2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinMnplus2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinMnplus2)){ output(spec->coord, spec->numPart, Out); }
