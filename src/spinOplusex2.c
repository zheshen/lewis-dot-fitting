...
extern double U_spinOplusex2;					//declare structure energy                           

void specspinOplusex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Oplusex2");			//Enter full name for the structure
	strcpy(spec->name, "O+1**");				//Enter abbreviation name
	strcpy(spec->spin, "a5b0");		//Enter spin structure
	spec->type = Oplusex2;				//Enter type name
	...
	spec->ainter = ainterspinOplusex2;			//Enter the function name for "ainter" 
	spec->dev = devspinOplusex2;			//Enter the function name for "dev"
...
double ainterspinOplusex2(Species * spec){
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//calculate structure energy
        if((MC_spinOplusex2) && (spec->icue ==4)){
...
double devspinOplusex2(Species *spec){
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinOplusex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinOplusex2)){ output(spec->coord, spec->numPart, Out); }
