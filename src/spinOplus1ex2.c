...
extern double U_spinOplus1ex2;					//declare structure energy                           

void specspinOplus1ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Oplus1ex2");			//Enter full name for the structure
	strcpy(spec->name, "O+1**");				//Enter abbreviation name
	strcpy(spec->spin, "a5b0");		//Enter spin structure
	spec->type = Oplus1ex2;				//Enter type name
	...
	spec->ainter = ainterspinOplus1ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinOplus1ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinOplus1ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinOplus1ex2)){ output(spec->coord, spec->numPart, Out); }
