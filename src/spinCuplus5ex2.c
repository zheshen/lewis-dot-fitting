...
extern double U_spinCuplus5ex2;					//declare structure energy                           

void specspinCuplus5ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus5ex2");			//Enter full name for the structure
	strcpy(spec->name, "Cu+5**");				//Enter abbreviation name
	strcpy(spec->spin, "a3b3");		//Enter spin structure
	spec->type = Cuplus5ex2;				//Enter type name
	...
	spec->ainter = ainterspinCuplus5ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus5ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
       ...
       //b-1 
        Vin[3+1].x = rot(rb1, 0.0, 0.0, a, b, c, 0);
        Vin[3+1].y = rot(rb1, 0.0, 0.0, a, b, c, 1);
        Vin[3+1].z = rot(rb1, 0.0, 0.0, a, b, c, 2);
        Vin[3+1].q = -1;
        Vin[3+1].s = -2;
       ...
       //b-2 
        Vin[3+2].x = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[3+2].y = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[3+2].z = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[3+2].q = -1;
        Vin[3+2].s = -2;
       ...
       //b-3 
        Vin[3+3].x = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[3+3].y = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[3+3].z = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[3+3].q = -1;
        Vin[3+3].s = -2;
        Vin[3+3].d = db1_r+db1_i*I;
        Vin[3+3].eta = db2;
        Vin[3+3].zeta = db3;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus5ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus5ex2)){ output(spec->coord, spec->numPart, Out); }
