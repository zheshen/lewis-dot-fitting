...
extern double U_spinCuplus4ex;					//declare structure energy                           

void specspinCuplus4ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus4ex");			//Enter full name for the structure
	strcpy(spec->name, "Cu+4*");				//Enter abbreviation name
	strcpy(spec->spin, "a4b3");		//Enter spin structure
	spec->type = Cuplus4ex;				//Enter type name
	...
	spec->ainter = ainterspinCuplus4ex;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus4ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus4ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus4ex)){ output(spec->coord, spec->numPart, Out); }
