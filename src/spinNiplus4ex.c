...
extern double U_spinNiplus4ex;					//declare structure energy                           

void specspinNiplus4ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus4ex");			//Enter full name for the structure
	strcpy(spec->name, "Ni+4*");				//Enter abbreviation name
	strcpy(spec->spin, "a4b2");		//Enter spin structure
	spec->type = Niplus4ex;				//Enter type name
	...
	spec->ainter = ainterspinNiplus4ex;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus4ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus4ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus4ex)){ output(spec->coord, spec->numPart, Out); }
