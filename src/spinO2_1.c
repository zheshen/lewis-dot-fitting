...
extern double U_spinO2_1;					//declare structure energy                           

void specspinO2_1(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "O2_1");			//Enter full name for the structure
	strcpy(spec->name, "O2-1");				//Enter abbreviation name
	strcpy(spec->spin, "a7b6");		//Enter spin structure
	spec->type = O2_1;				//Enter type name
	...
	spec->numPart = 15;					//Enter number of particles for the structure
	spec->stepSize = 0.101;					//Enter the stepsize for optimization, default is 0.101
	spec->ainter = ainterspinO2_1;			//Enter the function name for "ainter" 
	spec->dev = devspinO2_1;			//Enter the function name for "dev"
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinO2_1);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinO2_1)){ output(spec->coord, spec->numPart, Out); }
