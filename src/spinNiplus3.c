...
extern double U_spinNiplus3;					//declare structure energy                           

void specspinNiplus3(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus3");			//Enter full name for the structure
	strcpy(spec->name, "Ni+3");				//Enter abbreviation name
	strcpy(spec->spin, "a5b2");		//Enter spin structure
	spec->type = Niplus3;				//Enter type name
	...
	spec->ainter = ainterspinNiplus3;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus3;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus3);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus3)){ output(spec->coord, spec->numPart, Out); }
