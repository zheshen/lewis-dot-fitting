...
extern double U_spinCuplus4ex2;					//declare structure energy                           

void specspinCuplus4ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus4ex2");			//Enter full name for the structure
	strcpy(spec->name, "Cu+4**");				//Enter abbreviation name
	strcpy(spec->spin, "a6b1");		//Enter spin structure
	spec->type = Cuplus4ex2;				//Enter type name
	...
	spec->ainter = ainterspinCuplus4ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus4ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//a-4 
       ...
       //b-1 
        Vin[6+1].x = rot(0.0, 0.0, 0.0, a, b, c, 0);
        Vin[6+1].y = rot(0.0, 0.0, 0.0, a, b, c, 1);
        Vin[6+1].z = rot(0.0, 0.0, 0.0, a, b, c, 2);
        Vin[6+1].q = -1;
        Vin[6+1].s = -2;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus4ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus4ex2)){ output(spec->coord, spec->numPart, Out); }
