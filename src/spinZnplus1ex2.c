...
extern double U_spinZnplus1ex2;					//declare structure energy                           

void specspinZnplus1ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus1ex2");			//Enter full name for the structure
	strcpy(spec->name, "Zn+1^^");				//Enter abbreviation name
	strcpy(spec->spin, "a8b3");		//Enter spin structure
	spec->type = Znplus1ex2;				//Enter type name
	...
	spec->ainter = ainterspinZnplus1ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinZnplus1ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//a-4 
       ...
       //a-8 
        ...
        Vin[8].zeta = da3;
       //b-1 
        Vin[8+1].x = rot(rb1, 0.0, 0.0, a, b, c, 0);
        Vin[8+1].y = rot(rb1, 0.0, 0.0, a, b, c, 1);
        Vin[8+1].z = rot(rb1, 0.0, 0.0, a, b, c, 2);
        Vin[8+1].q = -1;
        Vin[8+1].s = -2;
        Vin[8+1].d = db1_r+db1_i*I;
        Vin[8+1].eta = db2;
        Vin[8+1].zeta = db3;

       //b-2 
        Vin[8+2].x = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[8+2].y = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[8+2].z = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[8+2].q = -1;
        Vin[8+2].s = -2;
        Vin[8+2].d = db1_r+db1_i*I;
        Vin[8+2].eta = db2;
        Vin[8+2].zeta = db3;

       //b-3 
        Vin[8+3].x = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[8+3].y = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[8+3].z = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[8+3].q = -1;
        Vin[8+3].s = -2;
        Vin[8+3].d = db1_r+db1_i*I;
        Vin[8+3].eta = db2;
        Vin[8+3].zeta = db3;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnplus1ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus1ex2)){ output(spec->coord, spec->numPart, Out); }
