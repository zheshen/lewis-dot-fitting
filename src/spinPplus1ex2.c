...
extern double U_spinPplus1ex2;					//declare structure energy                           

void specspinPplus1ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Pplus1ex2");			//Enter full name for the structure
	strcpy(spec->name, "P+1**");				//Enter abbreviation name
	strcpy(spec->spin, "a4b0");		//Enter spin structure
	spec->type = Pplus1ex2;				//Enter type name
	...
	spec->ainter = ainterspinPplus1ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinPplus1ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinPplus1ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinPplus1ex2)){ output(spec->coord, spec->numPart, Out); }
