...
extern double U_spinClneutrlex;					//declare structure energy                           

void specspinClneutrlex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Clneutrlex");			//Enter full name for the structure
	strcpy(spec->name, "Cl+0*");				//Enter abbreviation name
	strcpy(spec->spin, "a5b2");		//Enter spin structure
	spec->type = Clneutrlex;				//Enter type name
	...
	spec->ainter = ainterspinClneutrlex;			//Enter the function name for "ainter" 
	spec->dev = devspinClneutrlex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinClneutrlex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinClneutrlex)){ output(spec->coord, spec->numPart, Out); }
