...
extern double U_spinFeneutrlex2;					//declare structure energy                           

void specspinFeneutrlex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Feneutrlex2");			//Enter full name for the structure
	strcpy(spec->name, "Fe+0**");				//Enter abbreviation name
	strcpy(spec->spin, "a6b2");		//Enter spin structure
	spec->type = Feneutrlex2;				//Enter type name
	...
	spec->ainter = ainterspinFeneutrlex2;			//Enter the function name for "ainter" 
	spec->dev = devspinFeneutrlex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//a-4 
       ...
       //b-2 
        Vin[6+2].x = rot(0.0, -rb2, 0.0, a, b, c, 0);
        Vin[6+2].y = rot(0.0, -rb2, 0.0, a, b, c, 1);
        Vin[6+2].z = rot(0.0, -rb2, 0.0, a, b, c, 2);
        Vin[6+2].q = -1;
        Vin[6+2].s = -2;
        Vin[6+2].d = db2;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFeneutrlex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFeneutrlex2)){ output(spec->coord, spec->numPart, Out); }
