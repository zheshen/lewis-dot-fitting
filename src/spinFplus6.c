...
extern double U_spinFplus6;					//declare structure energy                           

void specspinFplus6(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Fplus6");			//Enter full name for the structure
	strcpy(spec->name, "F+6");				//Enter abbreviation name
	strcpy(spec->spin, "a1b0");		//Enter spin structure
	spec->type = Fplus6;				//Enter type name
	...
	spec->ainter = ainterspinFplus6;			//Enter the function name for "ainter" 
	spec->dev = devspinFplus6;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFplus6);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFplus6)){ output(spec->coord, spec->numPart, Out); }
