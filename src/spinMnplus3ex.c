...
extern double U_spinMnplus3ex;					//declare structure energy                           

void specspinMnplus3ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Mnplus3ex");			//Enter full name for the structure
	strcpy(spec->name, "Mn+3*");				//Enter abbreviation name
	strcpy(spec->spin, "a3b1");		//Enter spin structure
	spec->type = Mnplus3ex;				//Enter type name
	...
	spec->ainter = ainterspinMnplus3ex;			//Enter the function name for "ainter" 
	spec->dev = devspinMnplus3ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinMnplus3ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinMnplus3ex)){ output(spec->coord, spec->numPart, Out); }
