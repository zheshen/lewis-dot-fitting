...
extern double U_spinFplus3ex2;					//declare structure energy                           

void specspinFplus3ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Fplus3ex2");			//Enter full name for the structure
	strcpy(spec->name, "F+3**");				//Enter abbreviation name
	strcpy(spec->spin, "a4b0");		//Enter spin structure
	spec->type = Fplus3ex2;				//Enter type name
	...
	spec->ainter = ainterspinFplus3ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinFplus3ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFplus3ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFplus3ex2)){ output(spec->coord, spec->numPart, Out); }
