...
extern double U_spinCrplus2ex2;					//declare structure energy                           

void specspinCrplus2ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Crplus2ex2");			//Enter full name for the structure
	strcpy(spec->name, "Cr+2**");				//Enter abbreviation name
	strcpy(spec->spin, "a2b2");		//Enter spin structure
	spec->type = Crplus2ex2;				//Enter type name
	...
	spec->ainter = ainterspinCrplus2ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinCrplus2ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCrplus2ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCrplus2ex2)){ output(spec->coord, spec->numPart, Out); }
