...
extern double U_spinNiplus6;					//declare structure energy                           

void specspinNiplus6(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus6");			//Enter full name for the structure
	strcpy(spec->name, "Ni+6");				//Enter abbreviation name
	strcpy(spec->spin, "a4b0");		//Enter spin structure
	spec->type = Niplus6;				//Enter type name
	...
	spec->ainter = ainterspinNiplus6;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus6;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus6);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus6)){ output(spec->coord, spec->numPart, Out); }
