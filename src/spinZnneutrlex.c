...
extern double U_spinZnneutrlex;					//declare structure energy                           

void specspinZnneutrlex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znneutrlex");			//Enter full name for the structure
	strcpy(spec->name, "Zn+0*");				//Enter abbreviation name
	strcpy(spec->spin, "a7b5");		//Enter spin structure
	spec->type = Znneutrlex;				//Enter type name
	...
	spec->ainter = ainterspinZnneutrlex;			//Enter the function name for "ainter" 
	spec->dev = devspinZnneutrlex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//a-4 
       ...
       //b-1 
        Vin[7+1].x = rot(0.0, rb1, 0.0, a, b, c, 0);
        Vin[7+1].y = rot(0.0, rb1, 0.0, a, b, c, 1);
        Vin[7+1].z = rot(0.0, rb1, 0.0, a, b, c, 2);
        Vin[7+1].q = -1;
        Vin[7+1].s = -2;
        Vin[7+1].d = db1_r+db1_i*I;
        Vin[7+1].eta = db2;
        Vin[7+1].zeta = db3;

       //b-2 
        Vin[7+2].x = rot(0.0, -rb1, 0.0, a, b, c, 0);
        Vin[7+2].y = rot(0.0, -rb1, 0.0, a, b, c, 1);
        Vin[7+2].z = rot(0.0, -rb1, 0.0, a, b, c, 2);
        Vin[7+2].q = -1;
        Vin[7+2].s = -2;
        Vin[7+2].d = db1_r+db1_i*I;
        Vin[7+2].eta = db2;
        Vin[7+2].zeta = db3;

       //b-3 
        Vin[7+3].x = rot(rb1, 0.0, 0.0, a, b, c, 0);
        Vin[7+3].y = rot(rb1, 0.0, 0.0, a, b, c, 1);
        Vin[7+3].z = rot(rb1, 0.0, 0.0, a, b, c, 2);
        Vin[7+3].q = -1;
        Vin[7+3].s = -2;
        Vin[7+3].d = db1_r+db1_i*I;
        Vin[7+3].eta = db2;
        Vin[7+3].zeta = db3;

       //b-4 
        Vin[7+4].x = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[7+4].y = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[7+4].z = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[7+4].q = -1;
        Vin[7+4].s = -2;
        Vin[7+4].d = db1_r+db1_i*I;
        Vin[7+4].eta = db2;
        Vin[7+4].zeta = db3;

       //b-5 
        Vin[7+5].x = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[7+5].y = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[7+5].z = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[7+5].q = -1;
        Vin[7+5].s = -2;
        Vin[7+5].d = db1_r+db1_i*I;
        Vin[7+5].eta = db2;
        Vin[7+5].zeta = db3;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnneutrlex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnneutrlex)){ output(spec->coord, spec->numPart, Out); }
