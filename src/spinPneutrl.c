...
extern double U_spinPneutrl;					//declare structure energy                           

void specspinPneutrl(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Pneutrl");			//Enter full name for the structure
	strcpy(spec->name, "P+0");				//Enter abbreviation name
	strcpy(spec->spin, "a4b1");		//Enter spin structure
	spec->type = Pneutrl;				//Enter type name
	...
	spec->ainter = ainterspinPneutrl;			//Enter the function name for "ainter" 
	spec->dev = devspinPneutrl;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinPneutrl);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinPneutrl)){ output(spec->coord, spec->numPart, Out); }
