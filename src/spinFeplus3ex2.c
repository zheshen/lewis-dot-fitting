...
extern double U_spinFeplus3ex2;					//declare structure energy                           

void specspinFeplus3ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Feplus3ex2");			//Enter full name for the structure
	strcpy(spec->name, "Fe+3**");				//Enter abbreviation name
	strcpy(spec->spin, "a3b2");		//Enter spin structure
	spec->type = Feplus3ex2;				//Enter type name
	...
	spec->ainter = ainterspinFeplus3ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinFeplus3ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
       ...
       //b-2 
        Vin[3+2].x = rot(0.0, -rb2, 0.0, a, b, c, 0);
        Vin[3+2].y = rot(0.0, -rb2, 0.0, a, b, c, 1);
        Vin[3+2].z = rot(0.0, -rb2, 0.0, a, b, c, 2);
        Vin[3+2].q = -1;
        Vin[3+2].s = -2;
        Vin[3+2].d = db2;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFeplus3ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFeplus3ex2)){ output(spec->coord, spec->numPart, Out); }
