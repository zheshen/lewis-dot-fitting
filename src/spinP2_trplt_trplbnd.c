...
extern double U_spinP2_trplt_trplbnd;					//declare structure energy                           

void specspinP2_trplt_trplbnd(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "P2_trplt_trplbnd");			//Enter full name for the structure
	strcpy(spec->name, "P2_tr_tr");				//Enter abbreviation name
	strcpy(spec->spin, "a6b4");		//Enter spin structure
	spec->type = P2_trplt_trplbnd;				//Enter type name
	...
	spec->ainter = ainterspinP2_trplt_trplbnd;			//Enter the function name for "ainter" 
	spec->dev = devspinP2_trplt_trplbnd;			//Enter the function name for "dev"
...
Vin[11].s = 2;
	...
	//calculate structure energy
        if((MC_spinP2_trplt_trplbnd) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinP2_trplt_trplbnd);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinP2_trplt_trplbnd)){ output(spec->coord, spec->numPart, Out); }
