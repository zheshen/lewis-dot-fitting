...
extern double U_spinMnplus1ex;					//declare structure energy                           

void specspinMnplus1ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Mnplus1ex");			//Enter full name for the structure
	strcpy(spec->name, "Mn+1*");				//Enter abbreviation name
	strcpy(spec->spin, "a5b1");		//Enter spin structure
	spec->type = Mnplus1ex;				//Enter type name
	...
	spec->ainter = ainterspinMnplus1ex;			//Enter the function name for "ainter" 
	spec->dev = devspinMnplus1ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinMnplus1ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinMnplus1ex)){ output(spec->coord, spec->numPart, Out); }
