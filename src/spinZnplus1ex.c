...
extern double U_spinZnplus1ex;					//declare structure energy                           

void specspinZnplus1ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus1ex");			//Enter full name for the structure
	strcpy(spec->name, "Zn+1*");				//Enter abbreviation name
	strcpy(spec->spin, "a7b4");		//Enter spin structure
	spec->type = Znplus1ex;				//Enter type name
	...
	spec->ainter = ainterspinZnplus1ex;			//Enter the function name for "ainter" 
	spec->dev = devspinZnplus1ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnplus1ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus1ex)){ output(spec->coord, spec->numPart, Out); }
