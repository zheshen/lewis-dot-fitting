...
extern double U_spinVneutrlex2;					//declare structure energy                           

void specspinVneutrlex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Vneutrlex2");			//Enter full name for the structure
	strcpy(spec->name, "V+0**");				//Enter abbreviation name
	strcpy(spec->spin, "a3b2");		//Enter spin structure
	spec->type = Vneutrlex2;				//Enter type name
	...
	spec->ainter = ainterspinVneutrlex2;			//Enter the function name for "ainter" 
	spec->dev = devspinVneutrlex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinVneutrlex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinVneutrlex2)){ output(spec->coord, spec->numPart, Out); }
