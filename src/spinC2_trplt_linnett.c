...
extern double U_spinC2_trplt_linnett;					//declare structure energy                           

void specspinC2_trplt_linnett(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "C2_trplt_linnett");			//Enter full name for the structure
	strcpy(spec->name, "C2_tr_li");				//Enter abbreviation name
	strcpy(spec->spin, "a5b3");		//Enter spin structure
	spec->type = C2_trplt_linnett;				//Enter type name
	...
	spec->ainter = ainterspinC2_trplt_linnett;			//Enter the function name for "ainter" 
	spec->dev = devspinC2_trplt_linnett;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//calculate structure energy
        if((MC_spinC2_trplt_linnett) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinC2_trplt_linnett);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinC2_trplt_linnett)){ output(spec->coord, spec->numPart, Out); }
