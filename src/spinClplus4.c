...
extern double U_spinClplus4;					//declare structure energy                           

void specspinClplus4(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Clplus4");			//Enter full name for the structure
	strcpy(spec->name, "Cl+4");				//Enter abbreviation name
	strcpy(spec->spin, "a2b1");		//Enter spin structure
	spec->type = Clplus4;				//Enter type name
	...
	spec->ainter = ainterspinClplus4;			//Enter the function name for "ainter" 
	spec->dev = devspinClplus4;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinClplus4);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinClplus4)){ output(spec->coord, spec->numPart, Out); }
