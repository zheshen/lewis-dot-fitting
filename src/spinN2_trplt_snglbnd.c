...
extern double U_spinN2_trplt_snglbnd;					//declare structure energy                           

void specspinN2_trplt_snglbnd(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "N2_trplt_snglbnd");			//Enter full name for the structure
	strcpy(spec->name, "N2_tr_sn");				//Enter abbreviation name
	strcpy(spec->spin, "a6b4");		//Enter spin structure
	spec->type = N2_trplt_snglbnd;				//Enter type name
	...
	spec->ainter = ainterspinN2_trplt_snglbnd;			//Enter the function name for "ainter" 
	spec->dev = devspinN2_trplt_snglbnd;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//calculate structure energy
        if((MC_spinN2_trplt_snglbnd) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinN2_trplt_snglbnd);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinN2_trplt_snglbnd)){ output(spec->coord, spec->numPart, Out); }
