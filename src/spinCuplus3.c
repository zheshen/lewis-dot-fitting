...
extern double U_spinCuplus3;					//declare structure energy                           

void specspinCuplus3(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus3");			//Enter full name for the structure
	strcpy(spec->name, "Cu+3");				//Enter abbreviation name
	strcpy(spec->spin, "a5b3");		//Enter spin structure
	spec->type = Cuplus3;				//Enter type name
	...
	spec->ainter = ainterspinCuplus3;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus3;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus3);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus3)){ output(spec->coord, spec->numPart, Out); }
