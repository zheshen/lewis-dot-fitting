...
extern double U_spinZnplus1;					//declare structure energy                           

void specspinZnplus1(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus1");			//Enter full name for the structure
	strcpy(spec->name, "Zn+1");				//Enter abbreviation name
	strcpy(spec->spin, "a6b5");		//Enter spin structure
	spec->type = Znplus1;				//Enter type name
	...
	spec->ainter = ainterspinZnplus1;			//Enter the function name for "ainter" 
	spec->dev = devspinZnplus1;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnplus1);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus1)){ output(spec->coord, spec->numPart, Out); }
