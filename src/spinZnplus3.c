...
extern double U_spinZnplus3;					//declare structure energy                           

void specspinZnplus3(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus3");			//Enter full name for the structure
	strcpy(spec->name, "Zn+3");				//Enter abbreviation name
	strcpy(spec->spin, "a5b4");		//Enter spin structure
	spec->type = Znplus3;				//Enter type name
	...
	spec->ainter = ainterspinZnplus3;			//Enter the function name for "ainter" 
	spec->dev = devspinZnplus3;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnplus3);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus3)){ output(spec->coord, spec->numPart, Out); }
