...
extern double U_spinPplus3ex;					//declare structure energy                           

void specspinPplus3ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Pplus3ex");			//Enter full name for the structure
	strcpy(spec->name, "P+3*");				//Enter abbreviation name
	strcpy(spec->spin, "a2b0");		//Enter spin structure
	spec->type = Pplus3ex;				//Enter type name
	...
	spec->ainter = ainterspinPplus3ex;			//Enter the function name for "ainter" 
	spec->dev = devspinPplus3ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//calculate structure energy
        if((MC_spinPplus3ex) && (spec->icue ==4)){
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinPplus3ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinPplus3ex)){ output(spec->coord, spec->numPart, Out); }
