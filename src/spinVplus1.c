...
extern double U_spinVplus1;					//declare structure energy                           

void specspinVplus1(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Vplus1");			//Enter full name for the structure
	strcpy(spec->name, "V+1");				//Enter abbreviation name
	strcpy(spec->spin, "a4b0");		//Enter spin structure
	spec->type = Vplus1;				//Enter type name
	...
	spec->ainter = ainterspinVplus1;			//Enter the function name for "ainter" 
	spec->dev = devspinVplus1;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinVplus1);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinVplus1)){ output(spec->coord, spec->numPart, Out); }
