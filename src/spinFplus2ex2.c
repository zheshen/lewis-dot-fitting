...
extern double U_spinFplus2ex2;					//declare structure energy                           

void specspinFplus2ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Fplus2ex2");			//Enter full name for the structure
	strcpy(spec->name, "F+2**");				//Enter abbreviation name
	strcpy(spec->spin, "a5b0");		//Enter spin structure
	spec->type = Fplus2ex2;				//Enter type name
	...
	spec->ainter = ainterspinFplus2ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinFplus2ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFplus2ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFplus2ex2)){ output(spec->coord, spec->numPart, Out); }
