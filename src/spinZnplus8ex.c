...
extern double U_spinZnplus8ex;					//declare structure energy                           

void specspinZnplus8ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus8ex");			//Enter full name for the structure
	strcpy(spec->name, "Zn+8^");				//Enter abbreviation name
	strcpy(spec->spin, "a3b1");		//Enter spin structure
	spec->type = Znplus8ex;				//Enter type name
	...
	spec->ainter = ainterspinZnplus8ex;			//Enter the function name for "ainter" 
	spec->dev = devspinZnplus8ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnplus8ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus8ex)){ output(spec->coord, spec->numPart, Out); }
