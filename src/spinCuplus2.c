...
extern double U_spinCuplus2;					//declare structure energy                           

void specspinCuplus2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus2");			//Enter full name for the structure
	strcpy(spec->name, "Cu+2");				//Enter abbreviation name
	strcpy(spec->spin, "a5b4");		//Enter spin structure
	spec->type = Cuplus2;				//Enter type name
	...
	spec->ainter = ainterspinCuplus2;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus2)){ output(spec->coord, spec->numPart, Out); }
