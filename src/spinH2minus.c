...
extern double U_spinH2minus;					//declare structure energy                           

void specspinH2minus(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "H2minus");			//Enter full name for the structure
	strcpy(spec->name, "H2-1");				//Enter abbreviation name
	strcpy(spec->spin, "a2b1");		//Enter spin structure
	spec->type = H2minus;				//Enter type name
	...
	spec->ainter = ainterspinH2minus;			//Enter the function name for "ainter" 
	spec->dev = devspinH2minus;			//Enter the function name for "dev"
...
##################################################*/
	...
	Vin[4].q = -1;
	Vin[4].s = -2;
	Vin[4].d = db1_r*db1_i*I;


	//calculate structure energy
        if((MC_spinH2minus) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			//fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinH2minus);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinH2minus)){ output(spec->coord, spec->numPart, Out); }
