...
extern double U_spinTiplus1ex;					//declare structure energy                           

void specspinTiplus1ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Tiplus1ex");			//Enter full name for the structure
	strcpy(spec->name, "Ti+1*");				//Enter abbreviation name
	strcpy(spec->spin, "a2b1");		//Enter spin structure
	spec->type = Tiplus1ex;				//Enter type name
	...
	spec->ainter = ainterspinTiplus1ex;			//Enter the function name for "ainter" 
	spec->dev = devspinTiplus1ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinTiplus1ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinTiplus1ex)){ output(spec->coord, spec->numPart, Out); }
