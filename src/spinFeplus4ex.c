...
extern double U_spinFeplus4ex;					//declare structure energy                           

void specspinFeplus4ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Feplus4ex");			//Enter full name for the structure
	strcpy(spec->name, "Fe+4*");				//Enter abbreviation name
	strcpy(spec->spin, "a3b1");		//Enter spin structure
	spec->type = Feplus4ex;				//Enter type name
	...
	spec->ainter = ainterspinFeplus4ex;			//Enter the function name for "ainter" 
	spec->dev = devspinFeplus4ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
       ...
       //b-1 
        Vin[3+1].x = rot(0.0, 0.0, 0.0, a, b, c, 0);
        Vin[3+1].y = rot(0.0, 0.0, 0.0, a, b, c, 1);
        Vin[3+1].z = rot(0.0, 0.0, 0.0, a, b, c, 2);
        Vin[3+1].q = -1;
        Vin[3+1].s = -2;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFeplus4ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFeplus4ex)){ output(spec->coord, spec->numPart, Out); }
