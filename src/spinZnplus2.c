...
extern double U_spinZnplus2;					//declare structure energy                           

void specspinZnplus2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus2");			//Enter full name for the structure
	strcpy(spec->name, "Zn+2");				//Enter abbreviation name
	strcpy(spec->spin, "a5b5");		//Enter spin structure
	spec->type = Znplus2;				//Enter type name
	...
	spec->ainter = ainterspinZnplus2;			//Enter the function name for "ainter" 
	spec->dev = devspinZnplus2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnplus2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus2)){ output(spec->coord, spec->numPart, Out); }
