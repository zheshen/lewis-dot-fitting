...
extern double U_spinCoplus2ex2;					//declare structure energy                           

void specspinCoplus2ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Coplus2ex2");			//Enter full name for the structure
	strcpy(spec->name, "Co+2**");				//Enter abbreviation name
	strcpy(spec->spin, "a6b1");		//Enter spin structure
	spec->type = Coplus2ex2;				//Enter type name
	...
	spec->ainter = ainterspinCoplus2ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinCoplus2ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCoplus2ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCoplus2ex2)){ output(spec->coord, spec->numPart, Out); }
