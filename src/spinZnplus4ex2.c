...
extern double U_spinZnplus4ex2;					//declare structure energy                           

void specspinZnplus4ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus4ex2");			//Enter full name for the structure
	strcpy(spec->name, "Zn+4**");				//Enter abbreviation name
	strcpy(spec->spin, "a6b2");		//Enter spin structure
	spec->type = Znplus4ex2;				//Enter type name
	...
	spec->ainter = ainterspinZnplus4ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinZnplus4ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnplus4ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus4ex2)){ output(spec->coord, spec->numPart, Out); }
