...
extern double U_spinNiplus1;					//declare structure energy                           

void specspinNiplus1(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus1");			//Enter full name for the structure
	strcpy(spec->name, "Ni+1");				//Enter abbreviation name
	strcpy(spec->spin, "a5b4");		//Enter spin structure
	spec->type = Niplus1;				//Enter type name
	...
	spec->ainter = ainterspinNiplus1;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus1;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus1);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus1)){ output(spec->coord, spec->numPart, Out); }
