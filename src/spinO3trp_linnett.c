...
extern double U_spinO3trp_linnett;					//declare structure energy                           

void specspinO3trp_linnett(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "O3trp_linnett");			//Enter full name for the structure
	strcpy(spec->name, "O3trp_li");				//Enter abbreviation name
	strcpy(spec->spin, "a10b8");		//Enter spin structure
	spec->type = O3trp_linnett;				//Enter type name
	...
	spec->ainter = ainterspinO3trp_linnett;			//Enter the function name for "ainter" 
	spec->dev = devspinO3trp_linnett;			//Enter the function name for "dev"
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			//fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinO3trp_linnett);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinO3trp_linnett)){ output(spec->coord, spec->numPart, Out); }
