...
extern double U_spinOplus;					//declare structure energy                           

void specspinOplus(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Oplus");			//Enter full name for the structure
	strcpy(spec->name, "O+1");				//Enter abbreviation name
	strcpy(spec->spin, "a4b1");		//Enter spin structure
	spec->type = Oplus;				//Enter type name
	...
	spec->ainter = ainterspinOplus;			//Enter the function name for "ainter" 
	spec->dev = devspinOplus;			//Enter the function name for "dev"
...
double ainterspinOplus(Species * spec){
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
       ...
       //b-1 
        Vin[4+1].x = rot(0.0, 0.0, 0.0, a, b, c, 0);
        Vin[4+1].y = rot(0.0, 0.0, 0.0, a, b, c, 1);
        Vin[4+1].z = rot(0.0, 0.0, 0.0, a, b, c, 2);
        Vin[4+1].q = -1;
        Vin[4+1].s = -2;
        Vin[4+1].d = db1;
...
double devspinOplus(Species *spec){
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                if (MC_spinOplus){
                         fprintf(OutFile,"| Lone A1 - %10s dist  |      A     |      -         |   -   |   D_Init (MC)  |    % 8.5f    |   N/A   |       -       |\n", spec->fullname, fr1);
                         fprintf(OutFile,"| Lone B1 - %10s dist  |      A     |      -         |   -   |   D_Init (MC)  |    % 8.5f    |   N/A   |       -       |\n", spec->fullname, fr2);
                         //fprintf(OutFile,"| Linear A - %10s dist |      A     |      -         |   -   |   D_Init (MC)  |    % 8.5f    |   N/A   |       -       |\n", spec->fullname, fr3);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinOplus);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinOplus)){ output(spec->coord, spec->numPart, Out); }
