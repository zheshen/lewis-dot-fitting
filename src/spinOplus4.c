...
extern double U_spinOplus4;					//declare structure energy                           

void specspinOplus4(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Oplus4");			//Enter full name for the structure
	strcpy(spec->name, "O+4");				//Enter abbreviation name
	strcpy(spec->spin, "a1b1");		//Enter spin structure
	spec->type = Oplus4;				//Enter type name
	...
	spec->ainter = ainterspinOplus4;			//Enter the function name for "ainter" 
	spec->dev = devspinOplus4;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinOplus4);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinOplus4)){ output(spec->coord, spec->numPart, Out); }
