...
extern double U_spinCoplus1ex;					//declare structure energy                           

void specspinCoplus1ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Coplus1ex");			//Enter full name for the structure
	strcpy(spec->name, "Co+1*");				//Enter abbreviation name
	strcpy(spec->spin, "a6b2");		//Enter spin structure
	spec->type = Coplus1ex;				//Enter type name
	...
	spec->ainter = ainterspinCoplus1ex;			//Enter the function name for "ainter" 
	spec->dev = devspinCoplus1ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCoplus1ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCoplus1ex)){ output(spec->coord, spec->numPart, Out); }
