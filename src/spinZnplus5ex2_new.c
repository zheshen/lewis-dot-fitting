...
extern double U_spinZnplus5ex2;              //Declare structure energy                           

void specspinZnplus5ex2(Species * spec){     //Define SPEC function for specific structure

        int i;

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus5ex2");                     //Enter full name for the structure
	strcpy(spec->name, "Zn+5**");                               //Enter abbreviation name
	strcpy(spec->spin, "a6b");				//Enter spin structure
	spec->type = Znplus5ex2;                                      //Enter type name
	...
	spec->ainter = ainterspinZnplus5ex2;                      //Enter the function name for "ainter" 
	spec->dev = devspinZnplus5ex2;                            //Enter the function name for "dev"
	Initialize(spec);
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                if (MC_spinZnplus5ex2){
                        //Unchanged from lowermainspinFneutrl.c, except for index changes for spec->coord[].
                         fprintf(OutFile,"| Lone B1 - Znplus5ex2ex dist      |      A     |      -         |   -   |   D_Init (MC)  |    % 8.5f    |   N/A   |       -       |\n", fr1);
                         fprintf(OutFile,"| Lone A1 - Znplus5ex2ex dist      |      A     |      -         |   -   |   D_Init (MC)  |    % 8.5f    |   N/A   |       -       |\n", fr2);
                         fprintf(OutFile,"| Linear A - Znplus5ex2ex dist     |      A     |      -         |   -   |   D_Init (MC)  |    % 8.5f    |   N/A   |       -       |\n", fr3);
		...
		printEnergy(U_spinZnplus5ex2);
	}

	//structure output

	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus5ex2)){ output(spec->coord, spec->numPart, Out); }
