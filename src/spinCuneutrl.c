...
extern double U_spinCuneutrl;					//declare structure energy                           

void specspinCuneutrl(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuneutrl");			//Enter full name for the structure
	strcpy(spec->name, "Cu+0");				//Enter abbreviation name
	strcpy(spec->spin, "a6b5");		//Enter spin structure
	spec->type = Cuneutrl;				//Enter type name
	...
	spec->ainter = ainterspinCuneutrl;			//Enter the function name for "ainter" 
	spec->dev = devspinCuneutrl;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//a-4 
       ...
       //b-1 
        Vin[6+1].x = rot(0.0, rb1, 0.0, a, b, c, 0);
        Vin[6+1].y = rot(0.0, rb1, 0.0, a, b, c, 1);
        Vin[6+1].z = rot(0.0, rb1, 0.0, a, b, c, 2);
        Vin[6+1].q = -1;
        Vin[6+1].s = -2;
       ...
       //b-2 
        Vin[6+2].x = rot(0.0, -rb1, 0.0, a, b, c, 0);
        Vin[6+2].y = rot(0.0, -rb1, 0.0, a, b, c, 1);
        Vin[6+2].z = rot(0.0, -rb1, 0.0, a, b, c, 2);
        Vin[6+2].q = -1;
        Vin[6+2].s = -2;
       ...
       //b-3 
        Vin[6+3].x = rot(rb1, 0.0, 0.0, a, b, c, 0);
        Vin[6+3].y = rot(rb1, 0.0, 0.0, a, b, c, 1);
        Vin[6+3].z = rot(rb1, 0.0, 0.0, a, b, c, 2);
        Vin[6+3].q = -1;
        Vin[6+3].s = -2;
       ...
       //b-4 
        Vin[6+4].x = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[6+4].y = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[6+4].z = rot(-0.5*rb1, 0.0, 0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[6+4].q = -1;
        Vin[6+4].s = -2;
        Vin[6+4].d = db1_r+db1_i*I;
        Vin[6+4].eta = db2;
        Vin[6+4].zeta = db3;

       //b-5 
        Vin[6+5].x = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 0);
        Vin[6+5].y = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 1);
        Vin[6+5].z = rot(-0.5*rb1, 0.0, -0.5*sqrt(3.0)*rb1, a, b, c, 2);
        Vin[6+5].q = -1;
        Vin[6+5].s = -2;
        Vin[6+5].d = db1_r+db1_i*I;
        Vin[6+5].eta = db2;
        Vin[6+5].zeta = db3;
	...
	//calculate structure energy
        if((MC_spinCuneutrl) && (spec->icue ==4)){
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuneutrl);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuneutrl)){ output(spec->coord, spec->numPart, Out); }
