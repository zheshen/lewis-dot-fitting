...
extern double U_spinO2_trplbnd;					//declare structure energy                           

void specspinO2_trplbnd(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "O2_trplbnd");			//Enter full name for the structure
	strcpy(spec->name, "O2_tr");				//Enter abbreviation name
	strcpy(spec->spin, "a7b5");		//Enter spin structure
	spec->type = O2_trplbnd;				//Enter type name
	...
	spec->ainter = ainterspinO2_trplbnd;			//Enter the function name for "ainter" 
	spec->dev = devspinO2_trplbnd;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//calculate structure energy
        if((MC_spinO2_trplbnd) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinO2_trplbnd);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinO2_trplbnd)){ output(spec->coord, spec->numPart, Out); }
