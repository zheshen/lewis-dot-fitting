...
extern double U_spinCl2_snglt_snglbnd;					//declare structure energy                           

void specspinCl2_snglt_snglbnd(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cl2_snglt_snglbnd");			//Enter full name for the structure
	strcpy(spec->name, "Cl2_sn_sn");				//Enter abbreviation name
	strcpy(spec->spin, "a7b7");		//Enter spin structure
	spec->type = Cl2_snglt_snglbnd;				//Enter type name
	...
	spec->ainter = ainterspinCl2_snglt_snglbnd;			//Enter the function name for "ainter" 
	spec->dev = devspinCl2_snglt_snglbnd;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//calculate structure energy
        if((MC_spinCl2_snglt_snglbnd) && (spec->icue ==4)){
		for(i=0;i<4;i++){
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	double fr1, bo;
	const double real_bl = 1.990;
	const double real_bo = 1.0; 

        //Current Positions
        if (MC_spinCl2_snglt_snglbnd){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCl2_snglt_snglbnd);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCl2_snglt_snglbnd)){ output(spec->coord, spec->numPart, Out); }
