...
extern double U_spinNiplus3ex;					//declare structure energy                           

void specspinNiplus3ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus3ex");			//Enter full name for the structure
	strcpy(spec->name, "Ni+3*");				//Enter abbreviation name
	strcpy(spec->spin, "a4b3");		//Enter spin structure
	spec->type = Niplus3ex;				//Enter type name
	...
	spec->ainter = ainterspinNiplus3ex;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus3ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus3ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus3ex)){ output(spec->coord, spec->numPart, Out); }
