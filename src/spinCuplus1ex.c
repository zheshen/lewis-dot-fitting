...
extern double U_spinCuplus1ex;					//declare structure energy                           

void specspinCuplus1ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cuplus1ex");			//Enter full name for the structure
	strcpy(spec->name, "Cu+1*");				//Enter abbreviation name
	strcpy(spec->spin, "a6b4");		//Enter spin structure
	spec->type = Cuplus1ex;				//Enter type name
	...
	spec->ainter = ainterspinCuplus1ex;			//Enter the function name for "ainter" 
	spec->dev = devspinCuplus1ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//a-4 
       ...
       //b-1 
        Vin[6+1].x = rot(rb1, rb1, rb1, a, b, c, 0);
        Vin[6+1].y = rot(rb1, rb1, rb1, a, b, c, 1);
        Vin[6+1].z = rot(rb1, rb1, rb1, a, b, c, 2);
        Vin[6+1].q = -1;
        Vin[6+1].s = -2;
       ...
       //b-2 
        Vin[6+2].x = rot(rb1, -rb1, -rb1, a, b, c, 0);
        Vin[6+2].y = rot(rb1, -rb1, -rb1, a, b, c, 1);
        Vin[6+2].z = rot(rb1, -rb1, -rb1, a, b, c, 2);
        Vin[6+2].q = -1;
        Vin[6+2].s = -2;
       ...
       //b-3 
        Vin[6+3].x = rot(-rb1, rb1, -rb1, a, b, c, 0);
        Vin[6+3].y = rot(-rb1, rb1, -rb1, a, b, c, 1);
        Vin[6+3].z = rot(-rb1, rb1, -rb1, a, b, c, 2);
        Vin[6+3].q = -1;
        Vin[6+3].s = -2;
       ...
       //b-4 
        Vin[6+4].x = rot(-rb1, -rb1, rb1, a, b, c, 0);
        Vin[6+4].y = rot(-rb1, -rb1, rb1, a, b, c, 1);
        Vin[6+4].z = rot(-rb1, -rb1, rb1, a, b, c, 2);
        Vin[6+4].q = -1;
        Vin[6+4].s = -2;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCuplus1ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCuplus1ex)){ output(spec->coord, spec->numPart, Out); }
