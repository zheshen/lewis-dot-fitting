...
extern double U_spinClplus2;					//declare structure energy                           

void specspinClplus2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Clplus2");			//Enter full name for the structure
	strcpy(spec->name, "Cl+2");				//Enter abbreviation name
	strcpy(spec->spin, "a4b1");		//Enter spin structure
	spec->type = Clplus2;				//Enter type name
	...
	spec->ainter = ainterspinClplus2;			//Enter the function name for "ainter" 
	spec->dev = devspinClplus2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinClplus2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinClplus2)){ output(spec->coord, spec->numPart, Out); }
