...
extern double U_spinNiplus3ex2;					//declare structure energy                           

void specspinNiplus3ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Niplus3ex2");			//Enter full name for the structure
	strcpy(spec->name, "Ni+3**");				//Enter abbreviation name
	strcpy(spec->spin, "a6b1");		//Enter spin structure
	spec->type = Niplus3ex2;				//Enter type name
	...
	spec->ainter = ainterspinNiplus3ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinNiplus3ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinNiplus3ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinNiplus3ex2)){ output(spec->coord, spec->numPart, Out); }
