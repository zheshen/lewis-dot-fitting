...
extern double U_spinOplusex;					//declare structure energy                           

void specspinOplusex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Oplusex");			//Enter full name for the structure
	strcpy(spec->name, "O+1*");				//Enter abbreviation name
	strcpy(spec->spin, "a3b2");		//Enter spin structure
	spec->type = Oplusex;				//Enter type name
	...
	spec->ainter = ainterspinOplusex;			//Enter the function name for "ainter" 
	spec->dev = devspinOplusex;			//Enter the function name for "dev"
...
double ainterspinOplusex(Species * spec){
...
double devspinOplusex(Species *spec){
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinOplusex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinOplusex)){ output(spec->coord, spec->numPart, Out); }
