...
extern double U_spinCneutrl;					//declare structure energy                           

void specspinCneutrl(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Cneutrl");			//Enter full name for the structure
	strcpy(spec->name, "C+0");				//Enter abbreviation name
	strcpy(spec->spin, "a3b1");		//Enter spin structure
	spec->type = Cneutrl;				//Enter type name
	...
	spec->ainter = ainterspinCneutrl;			//Enter the function name for "ainter" 
	spec->dev = devspinCneutrl;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCneutrl);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCneutrl)){ output(spec->coord, spec->numPart, Out); }
