...
double lowermainspinO2(){
	...
	char molecule[] = "OXYGEN  DIATOMIC TRP MIN";
	...
	Species dblbnd;
	Species trplbnd;
	linnett.type = O2_linnett;
	snglbnd.type = O2_snglbnd;
	dblbnd.type = O2_dblbnd;
	trplbnd.type = O2_trplbnd;
	specSwitch(&linnett);
	specSwitch(&snglbnd);
	specSwitch(&dblbnd);
	specSwitch(&trplbnd);
...
#pragma omp section 
					...
					if (wspinO2_linnett)
					(*linnett.ainter)(&linnett);
					else U_spinO2_linnett = 0.0;
				}
#pragma omp section 
				{
					if (wspinO2_snglbnd)
					(*snglbnd.ainter)(&snglbnd);
					else U_spinO2_snglbnd = 0.0;
				}
#pragma omp section 
				{
					if (wspinO2_dblbnd)
					(*dblbnd.ainter)(&dblbnd);
					else U_spinO2_dblbnd = 0.0;
				}
#pragma omp section 
				{
					if (wspinO2_trplbnd)
      					(*trplbnd.ainter)(&trplbnd);
					else U_spinO2_trplbnd = 0.0;
				}
			}
			}


//					U_spinO2_linnett = (*linnett.ainter)(&linnett);
//					U_spinO2_snglbnd = (*snglbnd.ainter)(&snglbnd);
//					U_spinO2_dblbnd = (*dblbnd.ainter)(&dblbnd);
//					U_spinO2_trplbnd = (*trplbnd.ainter)(&trplbnd);
//					printf("%f, %f, %f, %f\n", U_spinO2_linnett, U_spinO2_snglbnd, U_spinO2_dblbnd, U_spinO2_trplbnd);

			U_spinO2=fmin(fmin(fmin(U_spinO2_linnett,U_spinO2_snglbnd),U_spinO2_dblbnd),U_spinO2_trplbnd); //spinO2 (triplet)
			if (U_spinO2==U_spinO2_linnett)
			{
			...
			else if(U_spinO2==U_spinO2_snglbnd)
			{
			...
			else if(U_spinO2==U_spinO2_dblbnd)
			{
			...
			else if(U_spinO2==U_spinO2_trplbnd)
			{
...
//				deviation = 5E5;
        ...
        fprintf(OutFile, "| Potential Energy      |   kJ/mol   |      -         |   -   |   Linnett      |  % 7.5e  |", U_spinO2_linnett );
        fprintf(OutFile, "   N/A   |       -       |\n");

        fprintf(OutFile, "| Potential Energy      |   kJ/mol   |      -         |   -   |   Snglbnd      |  % 7.5e  |", U_spinO2_snglbnd );
        fprintf(OutFile, "   N/A   |       -       |\n");

        fprintf(OutFile, "| Potential Energy      |   kJ/mol   |      -         |   -   |   Dblbnd       |  % 7.5e  |", U_spinO2_dblbnd );
        fprintf(OutFile, "   N/A   |       -       |\n");

        fprintf(OutFile, "| Potential Energy      |   kJ/mol   |      -         |   -   |   Trplbnd      |  % 7.5e  |", U_spinO2_trplbnd );
        fprintf(OutFile, "   N/A   |       -       |\n");
				...
				printEnergy(U_spinO2);
                        ...
                        if ((writefinalpdb)&&(spec.icue==4)){output(spec.coord, spec.numPart, "spinO2_min_final.pdb");}
