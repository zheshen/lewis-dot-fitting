...
extern double U_spinO2_snglbnd;					//declare structure energy                           

void specspinO2_snglbnd(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "O2_snglbnd");			//Enter full name for the structure
	strcpy(spec->name, "O2_sn");				//Enter abbreviation name
	strcpy(spec->spin, "a7b5");		//Enter spin structure
	spec->type = O2_snglbnd;				//Enter type name
	...
	spec->ainter = ainterspinO2_snglbnd;			//Enter the function name for "ainter" 
	spec->dev = devspinO2_snglbnd;			//Enter the function name for "dev"
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinO2_snglbnd);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinO2_snglbnd)){ output(spec->coord, spec->numPart, Out); }
