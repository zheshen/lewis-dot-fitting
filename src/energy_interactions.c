...
//		double anglePart = 1 + sin(pi / 2 + pi * C);//newly added in DE5045
		uA = E2_const*2.0/rootpi*pAB20*sqrt(2*Rhat)*exp(-pAB21*Rbar*r2)*exp(-pAB24*(Eta_a+Eta_b))*exp(-pAB25*(Zeta_a+Zeta_b));
...
//	Ihat = 0.5*(cimag(d_a)*cimag(d_a)/creal(d_a)+cimag(d_b)*cimag(d_b)/creal(d_b));
	...
	deff = p1*sqrt(1.0/Rbar)+p2*sqrt(Zeta_a+Zeta_b);
	reff = r/deff;
	reff2 = reff*reff;
	double q;
	q = a->q * b->q;
...
//	uAB = E2_const*M(d_a, r_a, d_b, r_b);	
	uexK = E_const*p14*Rbar*exp(-p15*Rbar*r2)*exp(-p16*(Eta_a+Eta_b))*exp(-p22*(Zeta_a+Zeta_b));
	uexR = -E2_const*2.0/rootpi*p17*sqrt(Rbar)*exp(-p18*Rbar*r2)*exp(-p19*(Eta_a+Eta_b))*exp(-p23*(Zeta_a+Zeta_b));
...
//	printf("Rq = %f, aq = %f, q = %f\n", R->q, a->q, q);
	...
	Eta_a = a->eta;
	Zeta_a = a->zeta;
	deff = p1 * (1.0/sqrt(creal(d_a))/sqrt(1.0+p7*Eta_a)+p2/sqrt(1.0+p8*Eta_a));
	reff = r/deff;
	reff2 = reff * reff;
