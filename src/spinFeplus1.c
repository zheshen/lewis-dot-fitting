...
extern double U_spinFeplus1;					//declare structure energy                           

void specspinFeplus1(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Feplus1");			//Enter full name for the structure
	strcpy(spec->name, "Fe+1");				//Enter abbreviation name
	strcpy(spec->spin, "a5b2");		//Enter spin structure
	spec->type = Feplus1;				//Enter type name
	...
	spec->ainter = ainterspinFeplus1;			//Enter the function name for "ainter" 
	spec->dev = devspinFeplus1;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
       ...
       //b-2 
        Vin[5+2].x = rot(0.0, -rb2, 0.0, a, b, c, 0);
        Vin[5+2].y = rot(0.0, -rb2, 0.0, a, b, c, 1);
        Vin[5+2].z = rot(0.0, -rb2, 0.0, a, b, c, 2);
        Vin[5+2].q = -1;
        Vin[5+2].s = -2;
        Vin[5+2].d = db2;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFeplus1);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFeplus1)){ output(spec->coord, spec->numPart, Out); }
