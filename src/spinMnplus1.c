...
extern double U_spinMnplus1;					//declare structure energy                           

void specspinMnplus1(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Mnplus1");			//Enter full name for the structure
	strcpy(spec->name, "Mn+1");				//Enter abbreviation name
	strcpy(spec->spin, "a6b0");		//Enter spin structure
	spec->type = Mnplus1;				//Enter type name
	...
	spec->ainter = ainterspinMnplus1;			//Enter the function name for "ainter" 
	spec->dev = devspinMnplus1;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinMnplus1);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinMnplus1)){ output(spec->coord, spec->numPart, Out); }
