...
extern double U_spinConeutrlex;					//declare structure energy                           

void specspinConeutrlex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Coneutrlex");			//Enter full name for the structure
	strcpy(spec->name, "Co+0*");				//Enter abbreviation name
	strcpy(spec->spin, "a5b4");		//Enter spin structure
	spec->type = Coneutrlex;				//Enter type name
	...
	spec->ainter = ainterspinConeutrlex;			//Enter the function name for "ainter" 
	spec->dev = devspinConeutrlex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
       ...
       //b-1 
        Vin[5+1].x = rot(rb1, rb1, rb1, a, b, c, 0);
        Vin[5+1].y = rot(rb1, rb1, rb1, a, b, c, 1);
        Vin[5+1].z = rot(rb1, rb1, rb1, a, b, c, 2);
        Vin[5+1].q = -1;
        Vin[5+1].s = -2;
       ...
       //b-2 
        Vin[5+2].x = rot(rb1, -rb1, -rb1, a, b, c, 0);
        Vin[5+2].y = rot(rb1, -rb1, -rb1, a, b, c, 1);
        Vin[5+2].z = rot(rb1, -rb1, -rb1, a, b, c, 2);
        Vin[5+2].q = -1;
        Vin[5+2].s = -2;
       ...
       //b-3 
        Vin[5+3].x = rot(-rb1, rb1, -rb1, a, b, c, 0);
        Vin[5+3].y = rot(-rb1, rb1, -rb1, a, b, c, 1);
        Vin[5+3].z = rot(-rb1, rb1, -rb1, a, b, c, 2);
        Vin[5+3].q = -1;
        Vin[5+3].s = -2;
       ...
       //b-4 
        Vin[5+4].x = rot(-rb1, -rb1, rb1, a, b, c, 0);
        Vin[5+4].y = rot(-rb1, -rb1, rb1, a, b, c, 1);
        Vin[5+4].z = rot(-rb1, -rb1, rb1, a, b, c, 2);
        Vin[5+4].q = -1;
        Vin[5+4].s = -2;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinConeutrlex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinConeutrlex)){ output(spec->coord, spec->numPart, Out); }
