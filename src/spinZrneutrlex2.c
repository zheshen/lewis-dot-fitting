...
extern double U_spinZrneutrlex2;					//declare structure energy                           

void specspinZrneutrlex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Zrneutrlex2");			//Enter full name for the structure
	strcpy(spec->name, "Zr+0**");				//Enter abbreviation name
	strcpy(spec->spin, "a2b2");		//Enter spin structure
	spec->type = Zrneutrlex2;				//Enter type name
	...
	spec->ainter = ainterspinZrneutrlex2;			//Enter the function name for "ainter" 
	spec->dev = devspinZrneutrlex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZrneutrlex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZrneutrlex2)){ output(spec->coord, spec->numPart, Out); }
