...
extern double U_spinF2_trplt_trplbnd;					//declare structure energy                           

void specspinF2_trplt_trplbnd(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "F2_trplt_trplbnd");			//Enter full name for the structure
	strcpy(spec->name, "F2_tr_tr");				//Enter abbreviation name
	strcpy(spec->spin, "a8b6");		//Enter spin structure
	spec->type = F2_trplt_trplbnd;				//Enter type name
	...
	spec->ainter = ainterspinF2_trplt_trplbnd;			//Enter the function name for "ainter" 
	spec->dev = devspinF2_trplt_trplbnd;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//calculate structure energy
        if((MC_spinF2_trplt_trplbnd) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinF2_trplt_trplbnd);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinF2_trplt_trplbnd)){ output(spec->coord, spec->numPart, Out); }
