...
extern double U_spinZnplus3ex2;					//declare structure energy                           

void specspinZnplus3ex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Znplus3ex2");			//Enter full name for the structure
	strcpy(spec->name, "Zn+3^^");				//Enter abbreviation name
	strcpy(spec->spin, "a7b2");		//Enter spin structure
	spec->type = Znplus3ex2;				//Enter type name
	...
	spec->ainter = ainterspinZnplus3ex2;			//Enter the function name for "ainter" 
	spec->dev = devspinZnplus3ex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinZnplus3ex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinZnplus3ex2)){ output(spec->coord, spec->numPart, Out); }
