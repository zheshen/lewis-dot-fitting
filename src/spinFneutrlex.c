...
extern double U_spinFneutrlex;					//declare structure energy                           

void specspinFneutrlex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Fneutrlex");			//Enter full name for the structure
	strcpy(spec->name, "F+0*");				//Enter abbreviation name
	strcpy(spec->spin, "a5b2");		//Enter spin structure
	spec->type = Fneutrlex;				//Enter type name
	...
	spec->ainter = ainterspinFneutrlex;			//Enter the function name for "ainter" 
	spec->dev = devspinFneutrlex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFneutrlex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFneutrlex)){ output(spec->coord, spec->numPart, Out); }
