...
extern double U_spinH2plus;					//declare structure energy                           

void specspinH2plus(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "H2plus");			//Enter full name for the structure
	strcpy(spec->name, "H2+1");				//Enter abbreviation name
	strcpy(spec->spin, "a1b0");		//Enter spin structure
	spec->type = H2plus;				//Enter type name
	...
	spec->ainter = ainterspinH2plus;			//Enter the function name for "ainter" 
	spec->dev = devspinH2plus;			//Enter the function name for "dev"
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			//fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinH2plus);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinH2plus)){ output(spec->coord, spec->numPart, Out); }
