...
extern double U_spinClplus1;					//declare structure energy                           

void specspinClplus1(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Clplus1");			//Enter full name for the structure
	strcpy(spec->name, "Cl+1");				//Enter abbreviation name
	strcpy(spec->spin, "a4b2");		//Enter spin structure
	spec->type = Clplus1;				//Enter type name
	...
	spec->ainter = ainterspinClplus1;			//Enter the function name for "ainter" 
	spec->dev = devspinClplus1;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinClplus1);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinClplus1)){ output(spec->coord, spec->numPart, Out); }
