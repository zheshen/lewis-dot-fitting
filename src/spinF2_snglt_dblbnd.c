...
extern double U_spinF2_snglt_dblbnd;					//declare structure energy                           

void specspinF2_snglt_dblbnd(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "F2_snglt_dblbnd");			//Enter full name for the structure
	strcpy(spec->name, "F2_sn_db");				//Enter abbreviation name
	strcpy(spec->spin, "a7b7");		//Enter spin structure
	spec->type = F2_snglt_dblbnd;				//Enter type name
	...
	spec->ainter = ainterspinF2_snglt_dblbnd;			//Enter the function name for "ainter" 
	spec->dev = devspinF2_snglt_dblbnd;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//calculate structure energy
        if((MC_spinF2_snglt_dblbnd) && (spec->icue ==4)){
		for(i=0;i<4;i++){
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
			...
			fprintf(OutFile,"| Bond Order            |      -     |   % 8.2f     |   -   |        -       |    %8.2f    |   N/A   |  % 8.2e    |\n", real_bo, bo,deviation*100);
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinF2_snglt_dblbnd);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinF2_snglt_dblbnd)){ output(spec->coord, spec->numPart, Out); }
