...
extern double U_spinP_1;					//declare structure energy                           

void specspinP_1(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "P_1");			//Enter full name for the structure
	strcpy(spec->name, "P-1");				//Enter abbreviation name
	strcpy(spec->spin, "a4b2");		//Enter spin structure
	spec->type = P_1;				//Enter type name
	...
	spec->ainter = ainterspinP_1;			//Enter the function name for "ainter" 
	spec->dev = devspinP_1;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinP_1);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinP_1)){ output(spec->coord, spec->numPart, Out); }
