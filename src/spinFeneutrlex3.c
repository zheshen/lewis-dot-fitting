...
extern double U_spinFeneutrlex3;					//declare structure energy                           

void specspinFeneutrlex3(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Feneutrlex3");			//Enter full name for the structure
	strcpy(spec->name, "Fe+0***");				//Enter abbreviation name
	strcpy(spec->spin, "a7b1");		//Enter spin structure
	spec->type = Feneutrlex3;				//Enter type name
	...
	spec->ainter = ainterspinFeneutrlex3;			//Enter the function name for "ainter" 
	spec->dev = devspinFeneutrlex3;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	//a-4 
        Vin[4].x = ra2*cos(2.0*pi/5.0);
        Vin[4].y = 0.0;
        Vin[4].z = ra2*sin(2.0*pi/5.0);
        Vin[4].q = -1;
        Vin[4].s = 2;
        Vin[4].d = da2;

        //a-5 
        Vin[5].x = ra2*cos(2.0*pi/5.0);
        Vin[5].y = 0.0;
        Vin[5].z = -ra2*sin(2.0*pi/5.0);
        Vin[5].q = -1;
        Vin[5].s = 2;
        Vin[5].d = da2;

       //a-6 
        Vin[6].x = -ra2*cos(pi/5.0);
        Vin[6].y = 0.0;
        Vin[6].z = ra2*sin(pi/5.0);
        Vin[6].q = -1;
        Vin[6].s = 2;
        Vin[6].d = da2;

       //a-7 
        Vin[7].x = -ra2*cos(pi/5.0);
        Vin[7].y = 0.0;
        Vin[7].z = -ra2*sin(pi/5.0);
        Vin[7].q = -1;
        Vin[7].s = 2;
        Vin[7].d = da2;
       //b-1 
        Vin[7+1].x = rot(0.0, 0.0, 0.0, a, b, c, 0);
        Vin[7+1].y = rot(0.0, 0.0, 0.0, a, b, c, 1);
        Vin[7+1].z = rot(0.0, 0.0, 0.0, a, b, c, 2);
        Vin[7+1].q = -1;
        Vin[7+1].s = -2;
        Vin[7+1].d = db1;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinFeneutrlex3);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinFeneutrlex3)){ output(spec->coord, spec->numPart, Out); }
