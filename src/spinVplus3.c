...
extern double U_spinVplus3;					//declare structure energy                           

void specspinVplus3(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Vplus3");			//Enter full name for the structure
	strcpy(spec->name, "V+3");				//Enter abbreviation name
	strcpy(spec->spin, "a2b0");		//Enter spin structure
	spec->type = Vplus3;				//Enter type name
	...
	spec->ainter = ainterspinVplus3;			//Enter the function name for "ainter" 
	spec->dev = devspinVplus3;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinVplus3);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinVplus3)){ output(spec->coord, spec->numPart, Out); }
