...
extern double U_spinScplus1ex;					//declare structure energy                           

void specspinScplus1ex(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Scplus1ex");			//Enter full name for the structure
	strcpy(spec->name, "Sc+1*");				//Enter abbreviation name
	strcpy(spec->spin, "a1b1");		//Enter spin structure
	spec->type = Scplus1ex;				//Enter type name
	...
	spec->ainter = ainterspinScplus1ex;			//Enter the function name for "ainter" 
	spec->dev = devspinScplus1ex;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinScplus1ex);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinScplus1ex)){ output(spec->coord, spec->numPart, Out); }
