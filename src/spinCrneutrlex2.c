...
extern double U_spinCrneutrlex2;					//declare structure energy                           

void specspinCrneutrlex2(Species * spec){				//define SPEC function for specific structure

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/

	strcpy(spec->fullname, "Crneutrlex2");			//Enter full name for the structure
	strcpy(spec->name, "Cr+0**");				//Enter abbreviation name
	strcpy(spec->spin, "a4b2");		//Enter spin structure
	spec->type = Crneutrlex2;				//Enter type name
	...
	spec->ainter = ainterspinCrneutrlex2;			//Enter the function name for "ainter" 
	spec->dev = devspinCrneutrlex2;			//Enter the function name for "dev"
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
       ...
       //b-1 
        Vin[4+1].x = rot(0.0, rb1, 0.0, a, b, c, 0);
        Vin[4+1].y = rot(0.0, rb1, 0.0, a, b, c, 1);
        Vin[4+1].z = rot(0.0, rb1, 0.0, a, b, c, 2);
        Vin[4+1].q = -1;
        Vin[4+1].s = -2;
       ...
       //b-2 
        Vin[4+2].x = rot(0.0, -rb1, 0.0, a, b, c, 0);
        Vin[4+2].y = rot(0.0, -rb1, 0.0, a, b, c, 1);
        Vin[4+2].z = rot(0.0, -rb1, 0.0, a, b, c, 2);
        Vin[4+2].q = -1;
        Vin[4+2].s = -2;
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW CHANGABLE>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	if (print){
		...
		OutFile = fopen(directory, "a");
                ...
                fprintf(OutFile, "|-------------------------------------------------------------------------------------------------------------------------|\n");
	    	fclose(OutFile);
		printEnergy(U_spinCrneutrlex2);
	}

	//structure output
	sprintf(Out, "MC_spin%s.pdb", spec->fullname);
        if ((pdbSwitch) && (MC_spinCrneutrlex2)){ output(spec->coord, spec->numPart, Out); }
