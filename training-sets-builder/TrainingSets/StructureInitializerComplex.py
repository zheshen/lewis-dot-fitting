...
def writediatomic(diatomic):
	for i in range(len(diatomic)):
		...
		q2 = int(diatomic[i][4])
		s2 = int(diatomic[i][5])
		spin_a = int(diatomic[i][6])
		spin_b = int(diatomic[i][7])
		real_bl = float(diatomic[i][8])
		real_bo = float(diatomic[i][9])
		FIX = int(diatomic[i][10])
...
##				     } 
		...
		text += "##################################################*/\n\n"
		if FIX == 1:
			text += FORMAT3%{'fullname':fullname,
					 'real_bl':real_bl,
					 'real_bo':real_bo,
					 'kernel':2,
					}
		elif FIX == 2:
			text += FORMAT4%{'fullname':fullname,
					 'real_bl':real_bl,
					 'real_bo':real_bo,
					 'kernel':2,
					}		 
		else:
			text += FORMAT5%{'fullname':fullname,
					 'real_bl':real_bl,
					 'real_bo':real_bo,
					 'kernel':2,
					}		 
...
def writetriatomic(triatomic):
	for i in range(len(triatomic)):
		...
		name = triatomic[i][1]
		q1 = int(triatomic[i][2])
		s1 = int(triatomic[i][3])
		q2 = int(triatomic[i][4])
		s2 = int(triatomic[i][5])
		q3 = int(triatomic[i][6])
		s3 = int(triatomic[i][7])
		spin_a = int(triatomic[i][8])
		spin_b = int(triatomic[i][9])
		real_bl = float(triatomic[i][10])
		real_bo = float(triatomic[i][11])
		FIX = int(triatomic[i][12])
